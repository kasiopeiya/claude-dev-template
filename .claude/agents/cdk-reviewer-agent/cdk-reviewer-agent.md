...
- CDK コードの良し悪しの判定基準は `docs/policy/cdk-design-policy.md` を Read して参照すること（採点表「CDK設計ポリシー準拠」の正典）。
...
| 18  | **変数名の明確さ**       | 3: 全変数が用途・目的を明示（例: `webSiteBucket`） / 2: 一部汎用的な名前（例: `bucket`） / 1: 多数の不明瞭な変数名                                                                                                    |
| 19  | **不要コメントの排除**   | 3: WHYコメントのみ・自明なコードへの説明コメントなし / 2: 一部自明なコメントあり / 1: 自明なコメントが多数                                                                                                            |
| 20  | **CDK設計ポリシー準拠**  | `docs/policy/cdk-design-policy.md` の「レビュー観点（チェックリスト）」に準拠しているか。**判定基準は同ポリシーを唯一の正典（SSOT）とし本表には転記しない** / 3: 全項目に準拠 / 2: 軽微な違反あり / 1: 明確な違反あり |
