...
- **スコアの扱い**: コードの採点には**含めない**。設計書の結果は report-format.md「設計書レビュー」セクションで文書ごとに別枠で報告する
...
| 8   | **Import 順序・配置**        | 3: 完全準拠（標準 → サードパーティ → 自作、かつ全 import がファイル冒頭に集約） / 2: 一部順序違反 / 1: 順序が乱れている、または途中 import がある                                                                                                                       |
...
| 11  | **不要なexport**             | 3: 不要な export なし / 2: 軽微な不要 export あり / 1: 不要な export が複数ある。モジュール外から参照されていないシンボルに `export` が付いていないか確認                                                                                                               |
| 12  | **不要なpublic公開**         | 3: 不要な public なし / 2: 軽微な不要 public あり / 1: 不要な public が複数ある。クラス外から参照されていないメンバ変数・メソッドに `public` が付いていないか確認。CDK の Stack/Construct が他のStack/Constructから参照されるプロパティは external usage ありと判断する |

### CDK 固有観点

| #   | 観点                     | スコアリング基準                                                                                                   |
| --- | ------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| 13  | **宣言的記述**           | 3: 宣言的 / 2: 一部 if/switch 使用 / 1: 制御構文の多用                                                             |
| 14  | **L2 Construct 優先**    | 3: L2 優先・L1 使用時は理由をコメントで記載 / 2: 一部 L1 使用 / 1: L1 多用                                         |
| 15  | **Import 形式統一**      | 3: `aws_* as service` 形式で完全統一 / 2: 一部違反 / 1: 統一されていない                                           |
| 16  | **IAM 自動生成活用**     | 3: `grant*` メソッド活用・明示的 Role 定義なし / 2: 一部明示的 Role / 1: 明示的 Role 多用                          |
| 17  | **リソース名自動生成**   | 3: 固定リソース名なし・CDK自動生成活用 / 2: 一部固定名あるが理由をコメントで記載 / 1: 理由なく多数の固定名指定     |
| 18  | **RemovalPolicy 明示**   | 3: Stateful リソースに適切に設定 / 2: 一部未設定 / 1: 未設定                                                       |
| 19  | **Construct ID 規則**    | 3: 自作 Construct 内のメインリソース ID が `Resource` or `Default` / 2: 一部違反 / 1: 規則違反                     |
| 20  | **スタック分割の適切さ** | 3: デプロイ単位として適切に分割 / 2: やや改善余地あり / 1: 分割が不適切                                            |
| 21  | **循環参照の検出**       | 3: 循環なし / 2: 潜在的リスク / 1: 循環参照あり                                                                    |
| 22  | **スタック複製可能性**   | 3: 同一アカウントに複数スタックをデプロイ可能 / 2: 一部固定名があるがプレフィックス付き / 1: 固定名により複製不可  |
| 23  | **環境分岐ロジック排除** | 3: スタック内に環境分岐なし / 2: 軽微な分岐あり / 1: `if (env === 'dev')` 等の環境分岐がスタック内に存在           |
| 24  | **パラメータ外部化**     | 3: 環境差異・外部依存が parameter.ts 等に集約 / 2: 一部スタック内にハードコード / 1: 設定値が各所に散在            |
| 25  | **変数名の明確さ**       | 3: 全変数が用途・目的を明示（例: `webSiteBucket`） / 2: 一部汎用的な名前（例: `bucket`） / 1: 多数の不明瞭な変数名 |
| 26  | **不要コメントの排除**   | 3: WHYコメントのみ・自明なコードへの説明コメントなし / 2: 一部自明なコメントあり / 1: 自明なコメントが多数         |
...
- **Import 順序・配置**: ファイル先頭の import 文を順番に確認し、標準ライブラリ → サードパーティ（CDK 含む）→ 自作モジュールの順を検証。さらに全 import がファイル冒頭に集約され、コード途中に import がないかを確認
...
全 CDK ファイルで**必ず** `.claude/rules/cdk.md` を Read し、そこに書かれた全ルールを1項目ずつ判定する。特に採点項目に対応しない「CDKの差分検知ルール」は素通りしやすい。これらは `cdk diff` の正確性・データ保護に関わる規約であり、違反は原則 **Critical**（必須）として報告する。
...
検出した違反は1件残らずレポートに反映する。スコア項目に対応するものはその項目の減点に反映し、対応項目が無いルール違反は report-format.md「📏 CDK ルールへの準拠」に ✅/⚠️/❌ で必ず明記する。

### Step 4: スコアリング

各観点を3点満点で評価し、**得点率**＝合計得点 ÷（適用観点数 × 3）でレベルを判定する。得点率で判定するため、観点を増減しても閾値の書き換えは不要である。
...
- **厳格・徹底**: 問題を見逃さない。「おそらく大丈夫」では採点しない。命名・スタイル・軽微な設定の緩さなど nit レベルも、引用が取れるものは漏らさず挙げる（severity は Critical/Recommended/Suggested で分類）
