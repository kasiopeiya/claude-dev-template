...
## インフラ設計書の内容レビュー（コードとは別枠）

コードに加え、設計ハブ `docs/design-hub.md` から辿れるインフラ設計書が存在する場合は、その**内容の妥当性**をレビューする。存在しなければスキップし、その旨を報告する。

- **基準**: `docs/policy/iac-infra-design-doc-policy.md` を Read し、その「レビュー観点」を判定基準として用いる
...
- **スコアの扱い**: コードの採点には**含めない**。設計書の結果は report-format.md「インフラ設計書レビュー」セクションで別枠で報告する
