...
コードに加え、インフラ設計書（既定パス `docs/design/infrastructure-design.md`）が存在する場合は、その**内容の妥当性**をレビューする。存在しなければスキップし、その旨を報告する。
...
- **スコアの扱い**: コードの 78 点満点スコアには**含めない**。設計書の結果は report-format.md「インフラ設計書レビュー」セクションで別枠で報告する
...
## レビュー観点・スコアリング基準（26項目・78点満点）

### TypeScript 共通観点（12項目・36点）
