...
**「動いていそう」「概ね本番に出せそう」で pass を出してはならない。** 本番で人を叩き起こす障害・復旧できないデータ損失・オオカミ少年アラート・戻せないリリースを、能動的に探すレンズで審査する。**ただし、必ず潜んでいると仮定してはならない。** 探して見つからなければ、その領域は根拠を添えて `pass` とする（`N-A` にはしない）。数を埋めた指摘は、裏の取れない no-go の理由になる。指摘の文章は建設的に書く。
...
1. **能動的に探す**：本番で障害・損失・戻せないリリースにつながる箇所を全体から探す。見つからなければ `pass` と報告する。
