...
| BR-07 | 手動調整上限 | 1回の手動付与は上限10,000ポイント。超過は部長承認が必要 | 行動 | — |
...
| 項目 | 除外理由 |
|---|---|
| ポイントを使ったキャンペーン抽選 | 目的（統合・失効自動化）に直結せず、別施策として切り出す |
| 会員ランク制度・ランク別付与率 | 付与ロジックが複雑化。まず統合を成立させることを優先（YAGNI） |
| 他社ポイントとの相互交換 | 提携先未定で要件が固まらない |

## 受入基準

各基準は検証対象（機能ID／業務ID／SLO）に紐づける。事後条件・SLO 目標値は再掲せず ID で参照する。
...
| ID | 未決事項 |
|---|---|
| OQ-1 | 失効処理の実行時間帯と POS 日次締め時刻の整合が未確認 |
| OQ-2 | 付与正確性 SLO の目標値・評価期間を顧客が未確定 |
| OQ-3 | キャンペーン提携先との SLA 合意の要否 |
| OQ-4 | 手動調整の承認フロー（部長承認の運用）を CS 部門と要確定 |
