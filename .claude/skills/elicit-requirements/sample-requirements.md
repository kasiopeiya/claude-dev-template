...
計算式・境界値は具体値まで書く。分類・区分や「AがBを持つ」関係を定めるルールには、多重度（0個を許すか・1つだけか複数か）と排他かどうかまで書く（BR-10 が例）。入力資料（RFC）にない記述・入力資料から変えた記述にだけ `出所` を付ける（そのまま採用したものは「—」）。ここでは BR-07 だけが作成者の補完で、顧客と未合意である。
