...
| タイプ   | 意味                                               | 例                                                                         |
| -------- | -------------------------------------------------- | -------------------------------------------------------------------------- |
| 行動     | 人（またはシステム）の行動を義務・禁止・条件づける | 「本システムは、常に、残高がマイナスになる利用要求を受け付けてはならない」 |
| 定義づけ | 分類の仕方・計算式・値の定義を定める               | 「税抜100円ごとに1ポイント（端数切り捨て、99円以下は0ポイント）」          |

`行動` は主体・条件・義務/禁止を書き切る（条件が無いなら「常に」と書く）。`定義づけ` は式・端数処理・境界値を具体値まで書き（端数処理が生じないなら「なし」と書く）、分類・区分や「AがBを持つ」関係には多重度（0個を許すか・1つだけか複数か）と排他かどうか、属性の値制約には必須/任意も添える。違反したときの扱い（拒否・保留）はここに書かず、機能仕様の「主要な例外」が持つ。入力資料（RFC）にない記述・入力資料から変えた記述にだけ `出所` を付ける（そのまま採用したものは「—」）。ここでは BR-07 だけが作成者の補完で、顧客と未合意である。
...
ここに書くのは、設計以降でないと決定できない未決事項と、名指しの例外である SLO 目標値だけである。要件段階で決めるべきそれ以外の事項は凍結までに決め、決まらなければ Issue で追跡する。各項目に解決の相手と解消期限を持たせ、出口の無い未決事項を残さない。解消期限は日付ではなく工程で書く。
