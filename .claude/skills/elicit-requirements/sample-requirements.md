...
`定義づけ` の書き分けは種類ごとに見る——BR-01・BR-03 は計算式（式・端数処理・境界値）、BR-06 は属性の値制約（取りうる値・必須/任意・桁・形式）、BR-10 は分類・区分（取りうる値・多重度・排他）の例である。`出所` が「—」でないのは BR-07 だけで、これは作成者の補完であり顧客と未合意である。
...
| F-07 | 稼働監視         | 業務維持／基盤（C. 監視の要求・SLO と対）                           | Must   | 付与・利用・失効の異常を検知できなければ金銭事故に気づけない                     |
| F-08 | データ保全       | 業務維持／基盤（A. 目標復旧時点（RPO）・C. バックアップの要求と対） | Must   | 残高データを失うと会員資産を毀損し回復できない                                   |
...
| F-03   | スケジューラ       | 日次起動（完了締切は B. 処理量・締切の目標） | 有効期限切れロット（内部データ）               | BR-03                             | 失効履歴（IO-04 に反映）                            | 対象ロットの残高が0になり失効履歴が残る                                              | 一部ロットの処理失敗→IO-08 で運用担当へ通知し残りは継続                                    |
| F-04   | 会員／店舗スタッフ | 照会要求受信                                 | IO-07                                          | —                                 | IO-04                                               | 参照のみ（状態を変えない）                                                           | 会員番号不明→該当なしを返す                                                                |
| F-05   | 運用担当           | 手動調整申請の承認                           | IO-06                                          | BR-07, BR-10                      | 調整後の残高・履歴（IO-04 に反映）・監査証跡（E）   | 調整ロットが発行/減算され、証跡が残る                                                | 上限超過（BR-07）→部長承認待ちで保留                                                       |
| F-06   | スケジューラ       | 月次起動                                     | 翌月末までに期限が到来するロット（内部データ） | BR-09                             | IO-05                                               | 対象会員への予告が配信依頼される                                                     | 配信失敗→失効日までに再送、なお未達なら IO-08 で通知                                       |
| F-07   | システム           | 監視対象の異常発生（対象は C. 監視の要求）   | 稼働状況（内部データ）                         | —                                 | IO-08                                               | 異常が運用担当に通知される                                                           | 通知手段の障害→未通知を検知できること（C. 監視の要求）                                     |
| F-08   | システム           | 常時（A. 目標復旧時点（RPO）を満たす鮮度で） | 残高・トランザクション（内部データ）           | —                                 | 復旧可能な保全データ（C. バックアップの要求に準拠） | 障害時に A. 目標復旧時間（RTO）・目標復旧時点（RPO）の範囲で復旧できる状態が保たれる | 保全の失敗→IO-08 で運用担当へ通知                                                          |
...
| 項目                | 要求                                                                            |
| ------------------- | ------------------------------------------------------------------------------- |
| サービス提供時間    | 付与・利用・残高照会は24時間365日（失効処理の完了締切は B. 処理量・締切の目標） |
...
| 大規模災害時の目標  | データセンター全域の被災時も復旧を目指す。目標復旧時間は24時間以内              |
...
| バックアップの要求     | ポイント残高・トランザクションを A. 目標復旧時点（RPO）を満たして復旧できるよう保全する。保全データは14日分保持 |
...
| 移行データ量           | 全会員分の残高・有効期限（会員数は B. 通常時の業務量）      |
...
| 追跡・監査ログの要求   | 手動調整（誰が・いつ・なぜ・いくら）を証跡として残す（保持期間は C. ログ保持の要求）                                                                           |
...
| 社内 IdP           | 運用担当の多要素認証                                      | E. 認証・認可の要求の前提            |
...
| 監視対象の異常（C. 監視の要求）が運用担当へ通知（IO-08）される                            | F-07      |
| 障害を想定した復旧で A. 目標復旧時間（RTO）・目標復旧時点（RPO）を満たす                  | F-08      |
...
| 失効処理が完了締切（B. 処理量・締切の目標）に間に合わない                                            | 締切を性能要件として明示し、想定データ量（入出力情報一覧）で達成できるかを設計段階で検証する                                                                                                                                     |
| Excel 移行データの残高不整合                                                                         | 移行前に突合し、差異は運用担当が確認・補正                                                                                                                                                                                       |
| メール配信サービスの送信上限超過で失効予告が届かない                                                 | 予告時期に余裕を持たせ（BR-09）、未達を失効前に検知して再送する                                                                                                                                                                  |
| ポイントが金銭価値を持つため、不正な付与で水増しを狙われる（手動調整の濫用・購入取消後も付与が残る） | 手動調整は申請と承認を分けて上限超過を保留し（BR-07）、誰が・いつ・なぜ・いくらを証跡に残す（E. 追跡・監査ログの要求）。購入取消分の付与残りは EC/POS との日次照合で検出し、付与正確性 SLI の逸脱として監視する（C. 監視の要求） |
