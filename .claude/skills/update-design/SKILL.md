...
2. 明示的な言及がなければ、設計ハブ `docs/design-hub.md` を Read し、リンクされている個別設計書の一覧と担当範囲を取得する。各設計書の担当範囲と、Issueのラベル・タイトル・スコープを照合し、最も関連する設計書を選ぶ
...
設計書を新規に書き起こす前に `samples/docs/design/` を Read する（構成・図の粒度・表の書きぶりの手本）。`samples/` 自体は変更しない。

**構成は基本方針・構成図の2本柱にする**（[design-doc.md](../../rules/design-doc.md)（Rule））。新規作成時はこの2つを柱に置き、個々の設計判断は中身を名指しした `##` セクションに立てる。

- **基本方針**：冒頭にこの設計が取る考え方を1文で言い切り、続けてその考え方から出てくる方針を箇条書きで並べる（表にしない。理由は添えず冒頭の1文が兼ねる）。設計の対象が2領域にまたがるなら `###` の小節（見出しはその方針の名前）に分け、1組ずつ置く。TL;DR は置かない（基本方針が兼ねる）
- **構成図**：コードから再構築するしかないもの（構成・関係・流れ）を描く。ノードには実装の固有名詞（ジョブ名・リソース名・クラス名）を書く。作図は `/design-doc-mermaid`
- **個々の判断**：その判断の**中身を見出しにした `##` セクション**を1つ立てる（例：`## CI実施内容一覧`・`## 命名規約`）。「重要なポイント」のような中身を名指ししない傘セクションは作らない。1セクション1決定。書くのは構成図に描かれていない内訳（実行条件の一覧・失敗時の分岐・状態の遷移・環境ごとの差）で、図の描き直しとコードの書き写しは書かない。同じ対象の列挙を他のセクションと重ねない

書かないもの：却下案（ADR 対象の判断で、なぜBでなくAか）は ADR へ出しリンクする。まだ実装されていないものは書かない（予定は Issue に置く）。コードを読めば分かることは書かない（一覧できず集約に価値がある場合のみ、概要を表に置く）。
