...
**構成は基本方針・構成図・重要なポイントの3本柱にする**（`docs/policy/design-doc-policy.md`）。新規作成時はこの3つを柱に置き、既存設計書の更新時も追加内容をこの3つのどれかに載せる。
...
| 重要なポイント | 1項目1行の表に**最大10個**まで並べ、**既定で各行を同名の `###` 節に展開する**（表は節の目次）。節に書くのは構成図に描かれていない内訳で、図の描き直しとコードの書き写しは書かない。表の行に留めてよいのは design-doc-policy が名指しする類型だけ。11個目を足すなら既存の1個と入れ替える |
