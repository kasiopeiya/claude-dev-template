...
- **新規ファイルを書き起こす前に `samples/app/backend/` を Read する。** レイヤーの切り方・命名・テストの粒度の手本がある。`samples/` 自体は絶対に変更しない
...
| 6   | 実装コードの異常系分岐の網羅 | 実装コードの異常系分岐（`throw`・エラーを表す戻り値・ガード句・バリデーション）を列挙し、対応するテストがあるか。テスト不要の分岐はテスト方針ポリシーの「テスト対象外」に名指しされたものだけ |
...
- **実装コードに異常系分岐があるが、対応するテストがない**：**不足**（設計書に記載が無くても同じ）
- **設計書にも実装コードの異常系分岐にも無いが、ビジネスロジック上テストすべき**：**推奨追加**（強制はしない）
