...
- **`app/` 配下のみを対象とする**
- **`infra/` は変更しない**（CDK は `/cdk-imp` の担当）
- **新規ファイルを書き起こす前に `samples/app/backend/` を Read する。** レイヤーの切り方・命名・テストの粒度の手本がある。`samples/` 自体は変更しない
...
ポリシーハブ `docs/policy-hub.md` を Read してテスト方針・テストダブル方針を定めるポリシー（例: `unit-test-policy.md`）を探し、Read で読み込む。書き方（AAA・命名・SUT・テストケースの分離・パラメータ化・フィクスチャ）は [unit-test.md](../../rules/unit-test.md)（Rule）を Read で読み込む。
...
さらに、設計ハブ `docs/design-hub.md` を Read し、Issueのラベル・タイトル・スコープと各設計書の担当範囲を照合して関連設計書（バックエンド仕様・フロントエンド仕様等）を特定・読み込み、テストケース設計の入力情報とする（特定ロジックは `update-design` スキルの Phase 2 に準じる）。
...
実装したテストファイルと実装コードを Read で読み込み、設計書から抽出した情報と照合する。
...
| 6   | テストレンズの指摘     | テストレンズ（`/code-review --lens テスト`）が指摘したテストケースの不足がないか（実行手順は下記）                         |

観点6は、Skill ツールで `/code-review` を起動して確認する:

```typescript
Skill({
  skill: 'code-review',
  args: '--lens テスト {実装したファイルパス}'
})
```
...
- **テストレンズ（`/code-review --lens テスト`）が指摘した不足**：**不足**（設計書に記載が無くても同じ）
- **設計書にもテストレンズの指摘にも無いが、ビジネスロジック上テストすべき**：**推奨追加**（強制はしない）
