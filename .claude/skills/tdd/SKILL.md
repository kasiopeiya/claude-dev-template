...
指定されたGitHub Issueからアプリケーションコード（CLAUDE.mdのディレクトリ構成が示すアプリケーションコードディレクトリ配下）のみを対象にTDDサイクル（Red-Green-Refactor）を実行してください。
CDK/インフラのディレクトリは絶対に実装・変更しないでください。
...
- **アプリケーションコードのディレクトリ（Phase 0 で読み取る）のみを対象とする**
- **CDK/インフラのディレクトリは絶対に実装・変更しないこと**
...
### Phase 0: 設定の発見（CLAUDE.md 起点）

**ポータビリティの原則**：ディレクトリ構成・設計書の所在をハードコードしない（プロジェクトごとに違う）。リポジトリ直下の `CLAUDE.md` を Read し、以下を読み取る。以降の全フェーズはこれを設定源とする。

- **アプリケーションコードのディレクトリ**（本スキルの実装対象）
- **CDK/インフラのディレクトリ**（本スキルの対象外・変更禁止）
- **設計ハブへのパス**（Phase 2 で関連設計書を特定する起点）
...
Phase 0 で読み取ったアプリケーションコードのディレクトリを対象とする（ラベルによる分岐は行わない。ラベルの語彙・粒度はプロジェクトごとに異なりうるため）。
...
- {アプリケーションコードのディレクトリ}/utils/state.ts
- {アプリケーションコードのディレクトリ}/utils/state.test.ts
```
...
```
question: "テスト対象ファイルとテストファイルのパスを入力してください。\n\n例:\n実装ファイル: {アプリケーションコードのディレクトリ}/utils/state.ts\nテストファイル: {アプリケーションコードのディレクトリ}/utils/state.test.ts"
header: "対象ファイル指定"
options: [
...
Phase 0 で読み取ったアプリケーションコードのディレクトリ配下を Glob（`**/*.ts`, `**/*.test.ts`, `**/*.tsx`, `**/*.test.tsx` 等）し、既存ファイルの配置パターン（同階層 or `__tests__/` サブディレクトリ等）を確認して踏襲する。
...
Phase 0 で読み取った CLAUDE.md からポリシーハブへのパスを特定し、それを Read してテスト方針・テストダブル方針を定めるポリシー（例: `unit-test-policy.md`）を探し、Read で読み込む。
...
| 2   | 検証手法の優先順位: 出力値ベース > 状態ベース > コミュニケーションベース                                             |
| 3   | 単体テストではモック・スタブを可能な限り使用しない                                                                   |
...
| 15  | テスト対象はビジネスロジック・事前条件チェック・サポートログのみ                                                     |
| 16  | フロントエンドはユーザーが認識できる属性で要素を取得（クエリの優先順位は [react.md](../../rules/react.md) が定める） |

さらに、Phase 0 で読み取った設計ハブを Read し、Issueのラベル・タイトル・スコープと各設計書の概要を照合して関連設計書（バックエンド仕様・フロントエンド仕様等）を特定・読み込み、テストケース設計の入力情報とする（特定ロジックは `update-design` スキルの Phase 2 に準じる）。
...
- テスト対象の判断基準に照らして、テストすべきコードかを先に判断する
...
- **テストダブル**: 単体テストではモック・スタブを使用しない（プロセス外依存と速度・決定性を壊す依存のみスタブ可、モックは統合テストのみ）
