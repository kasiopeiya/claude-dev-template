...
- **インフラ設計書・IaC 設計書**: 設計ハブ `docs/design-hub.md` を Read し、各設計書の概要からインフラ設計（何を組むか）と IaC 設計（どう作り・変えるか）を扱うものを**両方**特定して設計書レビューの対象に加える（片方しか無ければその1本、どちらも無ければ設計書レビューはスキップし、その旨を報告）
