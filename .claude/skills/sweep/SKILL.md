...
- **起票は必ず `/quick-issue` を呼ぶ。** 作業中に見つけた割れ窓・横展開の課題を起票するとき、`gh issue create` を自分で組み立てない。ラベル付与規則は quick-issue にしかなく、直叩きすると `boy-scout` が落ちて棚卸しの網から漏れる。本スキルは `gh` を直に叩く場面が多いので、その流れで起票まで直叩きに倒れやすい。
