...
指定されたGitHub Issueを元に、設計書を参照してAWS CDKコード（CLAUDE.mdのディレクトリ構成が示すCDK/インフラディレクトリ配下）のみを実装し、テスト・合成まで実行してください。
それ以外のディレクトリ（アプリケーションコード）は絶対に実装・変更しないでください。
...
- **CDK/インフラのディレクトリ（Phase 0 で読み取る）のみを対象とする**
- **それ以外のディレクトリ（アプリケーションコード）は絶対に実装・変更しないこと**
...
- **CDK/インフラのディレクトリ**（本スキルの実装対象）
- **アプリケーションコードのディレクトリ**（本スキルの対象外・変更禁止）
- **設計ハブへのパス**（Phase 2 で対象設計書を特定する起点）
...
Phase 0 で読み取った設計ハブ（例：`docs/design-hub.md`）を Read し、リンクされている個別設計書の一覧と概要を取得する。
