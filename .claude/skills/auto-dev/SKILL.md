---
name: auto-dev
description: Issue 1件を実装し、コミット・push・PR 本文の差し替えまで人間への確認なしに行う。`npm run auto-programmer` が AI 専用 clone の中で起動する。
...
Issue 1件を実装し、**push と PR 本文の差し替えまで**を人間への確認なしに自律実行する。起動するのは `npm run auto-programmer`（人間が直接打ってもよい）。
...
PR の URL・実行した検証・更新したタスクを表示して終了する。差分が無く PR を経ずに閉じたときは、URL の代わりに close した理由を表示する。
