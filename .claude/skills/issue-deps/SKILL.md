...
open Issue を、親・本文・ラベル・子の件数つきで一度に取ります。
...
段番号を振る対象から、次の2つを外します。

| 外すもの                                         | 理由                                       |
| ------------------------------------------------ | ------------------------------------------ |
| `phase` ラベルの常設Issue                        | 分類であって着手する対象ではない           |
| 子を持つ Issue（`subIssues.totalCount` が1以上） | 親は子をまとめる入れ物で、着手単位ではない |
...
グループの外を指す番号（別フェーズ・親Issue・`phase` ラベルIssue）は、**消さずに残して報告します。** 段の計算には入れません。書いた人が意図して置いた可能性があり、消すと理由ごと失われます。
...
書き出したファイルを Read し、Edit で `## ブロッカー` 節の中身だけを差し替えてから反映します。節が無い Issue には新しく足します。**置く位置は、その Issue を起票したテンプレートの節順に合わせます**（節順はテンプレート側が正典で、経路ごとに違います）。どのテンプレートか分からなければ本文の末尾に足します。
...
タイトルは、先頭の `N. ` を剥がしてから段番号を付け直します。

```bash
gh issue edit <番号> --title "<段>. <段番号を除いたタイトル>"
```

新しい段番号を付けない Issue は次の2つです。

| 対象             | 扱い                                                                          |
| ---------------- | ----------------------------------------------------------------------------- |
| close 済み Issue | タイトルに触らない。他の Issue の段を計算するときは、満たされた依存として扱う |
| 親Issue          | 番号を持っていたら（割る前に振られたもの）その番号だけを外す                  |

**`gh` を並列で叩かないでください。** 1件ずつ順に処理します。同じ Issue へ複数の更新が同時に飛ぶと、あとから投げたほうが前の更新を消します。

## Phase 5: 報告

グループごとに2つの表を出します。人間はこれを見て、今日どれに着手するかを決めます。
...
末尾に、タイトルを書き換えた件数（親から番号を外したものも数える）と `## ブロッカー` 節を直した件数を1行で添えます。
