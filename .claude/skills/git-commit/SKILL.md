...
- 分割の基準 → 「コミットの粒度」
- type / ブランチ prefix の語彙 → 「ブランチ命名規則」の一覧
...
type の語彙は git-policy.md「ブランチ命名規則」の prefix 一覧に従う（末尾の `/` を除いたものが type）。**まずファイル名・パス・ステータス（A/M）だけで判定を試み、それで確定しない場合のみ diff 内容を取得する**：
