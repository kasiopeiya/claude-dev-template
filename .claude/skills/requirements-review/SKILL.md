...
**`[警告]` で出た行は、レビュー本体へそのまま持ち込みます。** 警告は「ビジネスルールが1行も無い属性」の候補であって、違反ではありません（exit code も変えません）。ルールの不在は読んでも目に入らないので探索だけ機械にやらせ、違反かどうかは本文を読んで判定します。属性名の切り出しは自然文のパースなので、正当な記述が混じります。
