...
**着手前に必ず** `docs/design-hub.md`・`docs/policy-hub.md`（CLAUDE.md 「必読ドキュメント」）に加え、要件定義 `docs/requirements.md` を参照する。設計書ハブが未作成の場合はその旨を Plan に記録する。requirements.md は**要件の正典**であり、設計判断はこれに照らして下す。とくに **SLO**（レイテンシ・可用性・スループットの目標）は、設計の構造（同期/非同期・ネットワークホップ段数・冗長化）が目標を満たせるかを左右する制約なので必ず確認する。
