...
> [!IMPORTANT]
> **（AI・必須）** **変更の種類の定義**（それぞれのラベルが何を指すか）と、どの種類が人間レビューを要するか（レビュー方針の選定）は **[docs/policy/pr-review-policy.md](../../../docs/policy/pr-review-policy.md) の「PRラベルと変更の種類」「ラベルとレビュイーの経験でレビュー方針を決める」節が正**。本スキルはそれを**自動付与する手順だけ**を定義し、基準本文は再掲しない（二重管理を避けるため）。記述がポリシーと食い違うときは常にポリシーを正とする。
