...
付与したラベルと判定理由は常に**手元に表示**する。PR へのコメントは `needs-human-review` の有無で分ける。
...
- 判定根拠は **1〜2文**。トリガーとなったラベル名と、差分から読み取った**具体的な理由**を書く。ラベル名の再掲だけで終えない（「`feature` が付いたため」だけでは根拠にならない）。
- **`needs-manual-triage` も付けた場合**は、同じコメントに次のセクションを続ける（コメントを2件に分けない）。
