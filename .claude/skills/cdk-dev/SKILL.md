---
name: cdk-dev
description: 'CDK実装の一気通貫オーケストレーションワークフロー。CDK実装→コードレビュー→レビュー指摘修正→CI実行→CI指摘修正を人間への確認なしに自律実行する。CLAUDE.md の開発フロー Step 7（CDK実装）で使用。Issue番号を引数に指定すること（例: /cdk-dev 15）。アプリケーションコードの実装には使用しない。'
...
- 対象はCDK/インフラのディレクトリ（CLAUDE.mdのディレクトリ構成を参照）のみ。アプリケーションコードのディレクトリは絶対に実装・変更しない
