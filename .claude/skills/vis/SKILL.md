...
1. `mcp__Claude_Browser__resize_window` を `width: 900, height: 3000` で呼ぶ。1枚に入る量が増え、撮る枚数が減る
2. `mcp__Claude_Browser__preview_start` に `file:///<出力ファイルの絶対パス>` を渡す
3. `mcp__Claude_Browser__computer` の `screenshot` で見る。入りきらなければ下へ `scroll` して続きを撮る。**戻って撮り直さない**（同じ場所を往復すると枚数だけが増える）
4. 縮小されて細部が読めず、崩れが疑わしい節だけ、`resize_window` を `width: 900, height: 900` にして撮り直す
5. 下の崩れを直す
