...
### ⚠️ 同一ノード間の往復エッジのラベルが重なる

**Severity:** 🔴 Critical - 図として読めなくなる

**Applies to:** `flowchart` / `graph`（ハブ型＝1つのノードに複数の相手が集まる形で起きる）

**Problem:** 同じ2ノード間に `A -->|X| B` と `B -->|Y| A` を両方書くと、2本の線がほぼ同じ経路を通り、2つのラベルがほぼ同じ位置に置かれて重なる。ハブに集まる往復ペアが3組を超えると、隣の相手のラベルとも重なりはじめる。パースは成功するのでエラーは出ない。

**Incorrect（ラベルが重なる）:**

```text
flowchart TB
    ec[ECサイト] -->|購入確定イベント・ポイント利用要求| system[ポイント管理システム]
    system -->|付与/利用結果・残高| ec
    pos[POSシステム] -->|店舗購入・ポイント利用要求| system
    system -->|付与/利用結果・残高| pos
    member[会員] -->|残高・履歴の照会要求| system
    system -->|残高・失効予定の表示| member
    mail[メール配信サービス] -->|配信結果| system
...
```mermaid
flowchart TB
    ec[ECサイト] <-->|"送: 購入確定イベント・ポイント利用要求<br/>受: 付与/利用結果・残高"| system[ポイント管理システム]
    pos[POSシステム] <-->|"送: 店舗購入・ポイント利用要求<br/>受: 付与/利用結果・残高"| system
    member[会員] <-->|"送: 残高・履歴の照会要求<br/>受: 残高・失効予定の表示"| system
    mail[メール配信サービス] <-->|"送: 配信結果<br/>受: 失効予告の配信依頼"| system
```

**Fix:** 往復2本を `<-->` の1本に統合し、ラベルを `"送: X<br/>受: Y"` の2行にまとめる。線が1本になるとラベルの置き場所も1つになり、重なる相手が消える。**「送」「受」がどちらから見た向きかは図の直下の凡例で定義する**（例：「送＝外部から本システムへ、受＝本システムから外部へ」）。統合すると矢印の向きだけでは授受の区別が付かなくなるため、凡例が無いと情報が落ちる。

**効かない対処:**

| 対処案                       | 結果                                                                           |
| ---------------------------- | ------------------------------------------------------------------------------ |
| `direction` を `LR` に変える | ペアの数が減らないので重なりは残る（相手の並びが変わるだけ）                   |
| ラベルを短くする             | 重なりは消えるが、何を授受するかが読み取れなくなり、図の目的そのものが失われる |
