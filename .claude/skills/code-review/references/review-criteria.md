...
> [!IMPORTANT]
> **（AI・必須）** スコア項目は能動的に探す対象＝レビューの背骨である。ここには**機械（ESLint/knip）で測れない判断項目だけ**を載せる。`.claude/rules/typescript.md` の規約のうち ESLint/knip が error で落とせるもの（型 `any`・Import 順序・未使用宣言・関数長・引数数・マジックナンバー・ネスト深度など）は CI ゲートで担保し、本表の採点からは外す（次の「CI ゲートで機械的に担保される観点」参照）。機械化できない判断項目はスコア化しないと素通りするため、そのスコア化が品質の生命線となる。
