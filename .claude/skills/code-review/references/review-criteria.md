...
> [!IMPORTANT]
> **（AI・必須）** スコア項目は能動的に探す対象＝レビューの背骨である。ここには**機械（ESLint/knip）で測れない判断項目だけ**を載せる。書き方の規約のうち ESLint/knip/Prettier が error で落とせるもの（型 `any`・Import 順序・未使用宣言・関数長・引数数・マジックナンバー・ネスト深度など）は CI ゲートで担保し、本表の採点からは外す（次の「CI ゲートで機械的に担保される観点」参照）。機械化できない判断項目はスコア化しないと素通りするため、そのスコア化が品質の生命線となる。
...
| 観点                                                               | チェック内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | スコア基準                                                                                                                                                                                                          |
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **命名規則・変数名の妥当性**                                       | 変数名・関数名・定数名の明確性・一貫性。利用目的や用途が名前から想像できるか（❌ `mainFunc` ⭕ `accountFunc`）                                                                                                                                                                                                                                                                                                                                                                                                                  | 3点: 一貫性があり意図が明確 / 2点: 一部不明瞭 / 1点: 多数の不適切な命名                                                                                                                                             |
| **重複コード（DRY・文脈判断）**                                    | **DRY は常時遵守の原則ではない。**SRP を優先した意図的なコピペは許容する。重複が**偶有的（除去すべき）か本質的（無理に共通化すると誤った結合になる）か**を文脈で判断し、本質的な重複・誤った共通化の兆候のみ指摘する。文脈把握が要るため機械検知（jscpd 等）にはしない                                                                                                                                                                                                                                                          | 3点: 問題ある重複なし / 2点: 偶有的な重複が軽微にあり / 1点: 除去すべき重複が顕著                                                                                                                                   |
| **単一責任**                                                       | 関数・モジュールの責務の明確性。判定軸は `docs/policy/application-design-policy.md` の**一文テスト**（SSOT。判定基準は本表に転記しない）                                                                                                                                                                                                                                                                                                                                                                                        | 3点: 各単位の責務を一文で言える / 2点: やや複数の責任（一文に "and" が混ざる） / 1点: 明らかに複数の責任                                                                                                            |
| **不要な optional 宣言**                                           | 常に渡される引数・必ず存在するプロパティなのに `?` を付けていないか（未使用宣言そのものの検知は CI ゲートが担保。ここは「本来 required にすべきものを optional にしていないか」という設計判断に集中する）                                                                                                                                                                                                                                                                                                                       | 3点: 不要な optional なし / 2点: 軽微にあり / 1点: 明らかな不要 optional が複数                                                                                                                                     |
| **コメント**                                                       | `docs/policy/code-comment-policy.md` に準拠しているか（SSOT。実装コメントは WHY・doc comment は契約・ファイル冒頭コメントは責務と前提・名前で消せるコメントの有無）。具体基準はポリシー側に置き本表に転記しない                                                                                                                                                                                                                                                                                                                 | 3点: ポリシー準拠 / 2点: 一部不足または過多 / 1点: コメントなし or 過多                                                                                                                                             |
| **エラーハンドリング**                                             | 信頼境界から入る値の欠損・極値・型の逸脱の扱いが決まっているか（判定軸は `docs/policy/application-design-policy.md` の「信頼境界から入る値は、異常値の扱いを決めよ」。内部の関数どうしの引数への防御的チェックは求めない）。あわせてエラーメッセージが原因を伝えるか                                                                                                                                                                                                                                                            | 3点: 境界から入る値の扱いがすべて決まっている / 2点: 一部の値・区分で決まっていない / 1点: 境界から入る値を検証も判断もせず使っている                                                                               |
...
| **ロギングポリシー準拠**                                           | `docs/policy/application-logging-policy.md` に準拠しているか（SSOT）。判定基準はポリシー側に置き本表に転記しない。ログ出力を含まないファイルは対象外                                                                                                                                                                                                                                                                                                                                                                            | 3点: ポリシー準拠（ログ出力なしを含む） / 2点: 軽微な違反 / **1点: 重大な違反（個人情報・認証情報の出力等）**                                                                                                       |
| **フロントエンド設計ポリシー準拠**（フロントエンドコードのみ対象） | `docs/policy/frontend-design-policy.md` に準拠しているか（SSOT）。判定基準はポリシー側に置き本表に転記しない。非フロントエンドファイルは対象外（分母から除外）                                                                                                                                                                                                                                                                                                                                                                  | 3点: ポリシー準拠 / 2点: 軽微な違反 / 1点: 重大な違反                                                                                                                                                               |
| **単体テストポリシー準拠**（テストコードのみ対象）                 | `docs/policy/unit-test-policy.md` に準拠しているか（SSOT）。判定基準はポリシー側に置き本表に転記しない。テストファイル以外は対象外（分母から除外）                                                                                                                                                                                                                                                                                                                                                                              | 3点: ポリシー準拠 / 2点: 軽微な違反 / 1点: 重大な違反                                                                                                                                                               |
| **異常系テストの網羅**（実装コードのみ対象）                       | 実装の異常系分岐に、対応するテストがあるか。対応するテストファイルは `docs/policy/test-strategy-policy.md` の配置規約（`test/` 配下に実装のディレクトリ構造をミラーする）で実装ファイルのパスから引き、実装の分岐と1つずつ突き合わせる。判定基準（テストを書く分岐・書かない条件）は `docs/policy/unit-test-policy.md` を唯一の正典（SSOT）とし本表には転記しない。同ポリシー「テスト対象外」に当たる実装ファイルとテストファイル自体は対象外（分母から除外）。テストコード側の書き方は観点「単体テストポリシー準拠」が担当する | 3点: テストを書かない条件に当たるものを除き、全分岐にテストがある / 2点: テストの無い分岐があり、そのテストが無いことで素通りするバグを引用付きで示せる / 1点: 異常系テストが無い（テストファイルが無い場合を含む） |
