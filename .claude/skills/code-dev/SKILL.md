---
name: code-dev
description: 'アプリケーション実装の一気通貫ワークフロー。実装→コードレビュー→CI→設計書整合性チェックまでを人間への確認なしに自律実行する。Issue番号を引数に指定すること（例: /code-dev 15）。CDKには使用しない。'
...
2. **Phase 2: コードレビュー** - Phase 1 完了後、`code-review` スキルを Skill ツールで起動し、完了を待つ
3. **Phase 3: レビュー指摘修正とループ** - 詳細は下記「Phase 2〜3: レビューと修正のループ」
4. **Phase 4: CI実行** - `code-ci` スキルを Skill ツールで起動し、完了を待つ
5. **Phase 5: CI指摘修正** - Phase 4 の結果にエラーがある場合、静的解析・テストエラーを修正する。エラーなしの場合はスキップ
6. **Phase 6: 設計書整合性チェック** - `validate-design` スキルを Skill ツールで起動し、完了を待つ
...
`gh issue view $ARGUMENTS --json title,body,labels` で Issue を取得し、タスク一覧（`## タスク一覧` のチェックリスト）のうち **アプリケーションコードに関するタスク** を対象に、テストの新規作成・変更を伴うかを判定する。
...
- 指摘があれば、severity にかかわらずすべて直す（例外は同ポリシー「直す範囲は毎回すべて」の2種類だけ）。修正後 Phase 2 へ戻り `code-review` を再起動する。**再レビューには前回の指摘を渡さない**
