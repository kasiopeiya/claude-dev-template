...
| `ready-for-human` | 対応方針に人間の判断が要る | 人間が1件ずつ方針を決める対象を選ぶ          |

`ai-fixable` と `ready-for-human` は排他で、**必ずどちらか一方だけを付ける**。どちらも付かない Issue は、一斉対応からも個別相談からも漏れて誰の目にも触れなくなる。

### どちらを付けるか

分けるのは**対応方針が一意に定まるか**である。問題が明確でも、直し方が複数あるなら `ready-for-human` を付ける。
...
| 新しい判断基準をどの文書に置くか               | `ready-for-human` | 置き場所の選択が論点       |
| 検知漏れへの対策（rules 追加かガードレールか） | `ready-for-human` | 打ち手が複数あり比較が要る |

迷ったら `ready-for-human` を付ける。`ai-fixable` を誤れば人間が見ないまま方針が確定するが、逆は人間が1件確認するコストで済む（コストが非対称）。
...
- [ ] `ai-fixable` / `ready-for-human` のどちらか一方を決めた
...
gh issue create \
  --title "<タイトル>" \
  --assignee @me \
  --label boy-scout,<ai-fixable|ready-for-human> \
  --body "$(cat <<'EOF'
（テンプレートの全セクションを埋めた本文）
EOF
)"
