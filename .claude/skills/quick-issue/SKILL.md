...
書き先がまだ無いとき、設計書を勝手に新設してはならない。「人間に決めてほしいこと」の論点を「どの設計書を作って、そこに何を書くか」にする。設計書の新規作成は CLAUDE.md の開発フロー表で `/design` の担当であり、`/sweep` の一括処理で作ると、1 Issue 分の視野しか持たない設計書が既成事実になる。
