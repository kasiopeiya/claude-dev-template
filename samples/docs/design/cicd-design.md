...
- [重要なポイント](#重要なポイント)
  ...
  - [変更対象を「常時実行・全体・アプリ・CDK」に分類し、検査対象を絞る](#変更対象を常時実行全体アプリcdkに分類し検査対象を絞る)
  - [dev へのデプロイ経路を main の1本だけにする](#dev-へのデプロイ経路を-main-の1本だけにする)
  - [dev を触るものは、削除も含めて同じ直列化グループに入れる](#dev-を触るものは削除も含めて同じ直列化グループに入れる)
  - [PR の時点では deploy せず、`cdk diff` だけを確認する](#pr-の時点では-deploy-せずcdk-diff-だけを確認する)
  - [main への deploy が失敗したら GitHub Issue を自動起票する](#main-への-deploy-が失敗したら-github-issue-を自動起票する)
