...
マージするかどうかは `needs-human-review` ラベル1つで決まる。どの変更の種類が人間レビューを要するかは [pr-review-policy](../../../docs/policy/pr-review-policy.md) が正。  
...
| cicd-gateは上流が落ちても必ず実行し、上流の結果を1つずつ確かめる  | GitHub は「実行しなかった」を「成功」と同じものとして扱う。上流が skip されたまま素通りすると、検査していない PR がマージできてしまう                                                                                   |
...
| 不便                           | 内容                                                                                                                                                                                               |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| レビュー時点で deploy が未検証 | CDK は deploy しないと分からないエラーが多い。IAM の制約・リソース名の衝突・サービス上限は、main へ入った後の dev デプロイで初めて分かる。ここでは高速開発を優先し、失敗したらすぐ修正の PR を出す |
