...
> 例外は `docs/design/cicd-design.md` だけである。この設計書はテンプレート本体の `.github/` ワークフローが参照するため、ワークフローを変更したときは合わせて更新する。
...
| `docs`                               | 設計書（`design/cicd-design.md`・`design/infrastructure-design.md`・`design/iac-design.md`）と ADR（`adr/`） | 設計書の構成と図の粒度、ADR のフル版／軽量版の書き分け                     |
