...
| 症状                                             | 意味                                                                                                              | 直し方                                                      |
| ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| 「claude のセッションが … 回続けて失敗しました」 | claude がログイン切れ・利用上限などで動けない                                                                     | 記録の `exitCode`・`signal`・`error` を読んで直し、打ち直す |
...
| 「#… を飛ばします: 種類ラベル…」                 | Issue に変更の種類のラベルが無い（ブランチ名を決められない）                                                      | 種類ラベルを付ける（次の巡回で拾われる）                    |
