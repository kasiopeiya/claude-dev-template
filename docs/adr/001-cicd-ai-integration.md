...
現在の構成がなぜこの形で成り立つか（cicd-gate 後集約・required check の設計等）の詳細は cicd-design.md の「採用理由」が持つ。ここでは旧設計から変えた理由だけを記す。

## 検討した代替案（なぜ却下したか）

**決定の軸：PR 運用（説明の作成・レビュー要否判定・マージ）を AI に任せるか。**

| 案           | 内容                                     | 却下理由                                 |
| ------------ | ---------------------------------------- | ---------------------------------------- |
| 旧設計のまま | `--fill` を据え置き、AI 生成を導入しない | 目的（PR説明の AI 自動生成）を満たせない |

任せると決めた上での構成の選び方（AI を cicd-gate に入れる／自前の待機ワークフロー／PR説明の AI 生成のみ作成時実行 など）は、この軸への答えではない。それらの却下案は cicd-design.md の「却下案」が持つ。
