...
新規開発の初期に、プロジェクトのディレクトリ／パッケージ構成の方針を決める AI。
...
| ポート&アダプタ型                       | `domain/ application/ adapters(in/out)/`               | 差し替え可能性を構造で強制したい             | 小規模には過剰になりうる（必要になるまで作らない）            |
...
- **既定の方向**：構成には FW ではなくドメイン（何のためのシステムか）を語らせる（Screaming Architecture）。FW 既定の scaffold（レイヤー型）から始めても、成長を見越すなら機能・ドメイン型へ寄せる。
