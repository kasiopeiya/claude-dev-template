# Issueの階層ガイド

GitHub Issue を起票する AI が、その Issue をどの親にぶら下げるか・タイトル先頭の段番号をどう振るか決めたいときに読む。

> [!IMPORTANT]
> **TL;DR（このガイドの決定事項）**
>
> - 最上位はフェーズIssueで、Issue番号ではなく `phase` ラベルで探す
> - 分解元の Issue が無いときは**主に触るもの**でフェーズを決め、どのフェーズにも当たらないものは親を付けない
> - 段番号（着手順）は、open な子を持たない Issue にグループごとに1から振り、open な子を持つ親には子の段の範囲を付ける

## 階層の形

Issue は GitHub のネイティブ sub-issue で階層にする。sub-issue の親は1件しか持てないので、階層は分解の関係を表すことに使う。例外は最上位のフェーズIssueだけで、ここだけは分類である。

一度張った親子は原則として動かさない。動かしてよいのは1つの場合だけである。**親の open な子が全部ブロッカー待ちになったら**、その子をブロッカーが属する親へ移し、元の親を close する（[`/issue-regroup`](../../.claude/skills/issue-regroup/SKILL.md) が行う）。子1件の待ちで親が長く open に残ると、open な Issue の件数が残作業を表さなくなるためである。付け替えると要件へ戻る線が切れる。移す前に子へ「トレース（要件定義書との対応）」節を書き足し、書けない子は移さない。

```text
フェーズIssue（常設。名前は下の表で固定）
└ 起点Issue（新規開発ガイドが要件定義書から作る）
  └ スライスIssue（/to-issues が Plan から割る縦スライス）
    └ さらに分割した sub-issue
```

## 最上位のフェーズIssue

最上位は開発フローのフェーズを表す常設Issueで、名前は次の4つに固定する。探すときは `phase` ラベルを使い、Issue番号は使わない（番号はプロジェクトごとに変わるため）。

- **要件定義**：要件定義書に関するタスク。未決事項の解消
- **開発準備**：環境構築・技術スタック選定・全体構造設計（アプリアーキテクチャ・ディレクトリ構成）・CI／デプロイ経路の整備
- **設計・実装**：開発系のタスク全般。単体テストも含む
- **結合テスト**：結合テスト・E2Eテスト。テストケース作成とテスト用の環境整備も含む

フェーズIssueは close しない。`boy-scout`・`ai-fixable`・`issue:needs-human-decision` は付けない（付けると `/sweep`・`/issue-check` が毎回拾って空回りする）。まだ無いリポジトリでは、起票を始める前に上のフェーズをすべて作る。

```bash
gh label create phase --description "Issue階層の最上位。開発フローのフェーズを表す常設Issue" --color "0E4B99"
gh issue create --title "<フェーズ名>" --label phase --body "<上に挙げたそのフェーズのぶら下げるもの>"
```

## 親の決め方

分解元の Issue があるなら、それが親である。起点Issueの親は種別で決まり（[新規開発ガイドの種別表](../guide/new-development-guide.md#起票する起点issue)）、スライスIssue（`/to-issues` 産）は起点Issueの子にする。

起点Issueが無いまま `/to-issues` が2件以上を割ったときは、まとめ用の親Issueを作ってスライスIssueの親にする（作り方は `/to-issues` が正典）。

分解元が無い Issue——まとめ用の親Issue、起点Issueが無い1件だけのスライスIssue、単発Issue（`/quick-issue` 産）——は、**主に触るもの**でフェーズを決める。

- `docs/requirements.md`：要件定義
- **開発環境・技術スタック・アプリアーキテクチャ・ディレクトリ構成・`.github/workflows/`**：開発準備
- **`app/`・`infra/` のコードと、その単体テスト**：設計・実装
- **結合テスト・E2Eテストのコードと、その実行環境**：結合テスト
- **`.claude/` 配下・`docs/policy/`・`docs/design/`・`docs/guide/`**：親を付けない

判定を**触るパス**で書くのは、書いてあるか書いていないかで決まり、重要度の評価を挟まないからである。「関係がありそう」で選ばせると、AIの判定は必ず甘くなる。

いちばん多いのは最終行である。ハーネス・ポリシー・設計書の手直しは開発フローのどのフェーズの作業でもないので、親を付けずに起票し、`boy-scout` ラベルで一覧する。

## 段番号（着手順）

タイトルの先頭に付ける数字（例：`3. F-02 ポイント利用`）。Issue を開かなくても着手順が分かるようにするためにある。同じ段の Issue は互いに依存せず、並行して着手できる。

- **誰に振るか**：open な子を持たない Issue（着手単位）。open な子を持つ親Issueには、子孫の段の最小〜最大を付ける（例：`1〜3. 全体構造設計`）
- **どの範囲で1から振るか**：親を上に辿って行き着いたフェーズIssueごとに1から。親を持たない Issue は「親なし」でひとつのグループ
- **何で決まるか**：`## ブロッカー` 節。ブロッカーが無い（または全部 close 済み）なら段1、そうでなければブロッカーの段の最大値＋1。ブロッカーが親なら、その範囲の最大値を使う
- **close 済みの扱い**：タイトルを書き換えない。他の Issue の段を計算するときは、満たされた依存として扱う

<details>
<summary>規約をこう決めた理由</summary>

- **親Issueには1つの段でなく範囲を付ける**：親は着手する対象ではなく、子をまとめる入れ物である。1つの段を振ると、親をまたぐ子同士の順序（`1.` の親の子が `3.` の親の後になる形）が表せない。範囲なら子の番号と食い違わず、親がいつ動く塊なのかも一覧で読める。
- **子が全部 close 済みの親には振る**：入れ物の中身が空なので、残りを割るか閉じるかの次の着手単位に戻る。外すと、どの一覧にも現れなくなる。
- **グループごとに1から振る**：依存はフェーズをまたがない前提であり、フェーズ間の前後はフェーズそのものが表す。全体の通し番号は範囲が広すぎて、番号を見ても隣り合う Issue との関係が読めない。
- **close 済みのタイトルを書き換えない**：着手が進むたびに番号が繰り上がると、会話や PR で特定の Issue を番号で指せなくなる。

</details>

番号は起票時の見立てなので、着手が進むと実態からずれる。open Issue の本文から依存を推定し直し、`## ブロッカー` 節と段番号を振り直すのは [`/issue-deps`](../../.claude/skills/issue-deps/SKILL.md) の役目である。

## 紐づけるコマンド

```bash
# 親のフェーズIssueを node ID つきで取る
gh issue list --label phase --state open --json number,title,id

# 親子を張る（子はURLで指定できるので node ID に変換しなくてよい）
gh api graphql -f query='mutation($parent:ID!,$childUrl:String!){addSubIssue(input:{issueId:$parent,subIssueUrl:$childUrl}){subIssue{number}}}' \
  -F parent="<親のnode ID>" -F childUrl="<子のURL>"
```

紐付けに失敗しても、**起票そのものは成功している**。失敗したら親が付かなかったことを伝えて続行する。

## 上限に達したらフェーズIssueを世代交代させる

GitHub の sub-issue は親1件あたり100件・深さ8階層が上限で、超えると `addSubIssue` が失敗する。close した子は枠を空けない（GitHub が上限到達時に案内するのは「sub-issue の紐付けを外す」か「新しい親を作る」のどちらかで、close は含まれない）。

上限に当たったら、**同名に連番を付けた後継Issueを作り、以後の紐付け先をそちらに切り替える。**

```bash
gh issue create --title "<フェーズ名> (2)" --label phase \
  --body "<フェーズ表の「ぶら下げるもの」>。#<前のフェーズIssue番号> の後継。"
```

旧フェーズIssueは close せず履歴として残す。`phase` ラベルで探すのは変わらないので、**同じフェーズ名のIssueが複数見つかったら、いちばん番号が大きいものに付ける。**
