...
**概要**: 「良い設計とは何か」を技術・パターン非依存で宣言。コード内部設計（クラス・関数・モジュールの作り方）が対象。正しく使うのが一番易しいインターフェース・関数とクラスの使い分け・Primitive Obsession を避ける（ドメイン概念に専用型＝値オブジェクト）・共通化の判断・引数は責務の鏡（ブーリアン引数アンチパターン禁止）・失敗は投げて伝える。マクロ構造は application-architecture-policy に委譲
