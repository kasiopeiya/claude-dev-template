---
hook:
  applies-to: ['**/*monitoring*.ts', '**/*Monitoring*.ts', '**/*alarm*.ts', '**/*Alarm*.ts']
---

# モニタリング・アラームポリシー

監視・アラームの「良い／悪い」を判断するためのレビュー基準を示すドキュメント。**AIエージェントが、監視・アラームを設計する／作られた監視設定をレビューし、何をどう監視・アラート化すべきかの統一基準を知りたいとき**に参照する。
...
**各項目には「なぜそれを監視／アラート化するのか」の根拠が要る**。そして根拠は頭の中ではなく**文書化された形で残す**。残す場所は設計書が基本だが、CDK等のコードでアラームを定義していて設計書を作らない場合は、**コード内（コメント等）に書く形でもよい**。レビュアーが見るのは「根拠が（場所を問わず）書かれているか」と「その根拠が妥当か（目的に紐づくか）」である。

- ❌ 弱い：とりあえずCPU・メモリ・ディスクにアラームを張った（なぜそれかの根拠なし）
- ✅ 強い：「決済完了率」を監視する。根拠：本システムの目的は決済完了であり、その失敗を最も直接的に示す指標だから

> [!TIP]
> **（AI・推奨）** 根拠を目的から逆算して作る推奨手法として **GSMフレームワーク** がある。GSMの使用は必須ではないが、「何のために測るか」を起点に根拠を残せるため推奨する。
...
- **アラートは「エラーバジェットの消費速度（バーンレート）」で鳴らす（すべての SLI で必須）。** SLI/SLO はユーザーに約束したサービスレベルなので、逸脱＝ユーザー影響であり、種別で絞らず全て対象にする。鳴らす対象を SLO のしきい値そのものにしないのは、SLO（例：30日で 99.9%）を直接監視すると割れた時点で期間を使い切った後＝手遅れだから。バーンレートなら使い切る前に予兆を掴め、消費の速さで緊急度を分けられる（速い消費＝即ページ／遅い消費＝チケット。具体的なしきい値・バーンレートの観測ウィンドウ設計は実装の責務。SLO の評価期間そのものは要件で決める）。オオカミ少年は「アラートを絞る」のではなく次の2点で防ぐ——① 原因系メトリクス（CPU・キュー長等）は SLI にせずホワイトボックス＝ダッシュボードに置く、② 瞬間値のブレでなくバーンレートで鳴らす。
...
[外形監視（Synthetics）](#外形監視syntheticsは選定フローと独立した必須ベースライン)と違い、**RUM は全システム一律で導入するものではなく**、根拠（GSM 等）に基づいて採否を判断する。
...
> [!TIP]
> **（AI・推奨）** 街灯効果の根本原因は **「測れるもの（Metric）から考える」順序** にある。GSMが強制するのは **Goal → Signal → Metric**、つまり「起点を目的に置く」という**順序の逆転**である。この順序を守るだけで、大半の街灯問題は構造的に防げる（個々の指標を叩くより、選ぶ順序を直す方が効く）。街灯効果は「目的を見失えば、速く進むほど遠ざかる」（[refined-engineer-judgment-principles](refined-engineer-judgment-principles.md)）の計測版——手段（取りやすさ）が目的（何を知りたいか）に昇格した状態である。
...
鳴っても誰も動かないアラートは、有害な「オオカミ少年」である。アラートは**少なく・鋭く**保つ。
...
### アラームにレベルを付ける — crit / warn / info

アラームには**深刻度レベル**を付け、名前の**サフィックス**に埋め込む（例：`payment-success-rate-low-crit`）。レベルが名前に出ていれば、通知の振り分け先と対応の緊急度を、アラーム定義を開かずに読み取れる。

一般的な深刻度指標の **SEV（Severity）レベル** は5段階で粒度が細かく、「SEV3か4か」で迷いやすい。本プロジェクトは扱いやすさを優先し、SEVを**3レベルに畳んで**使う。

| レベル（suffix） | 対応するSEV | 意味                                         | 通知と対応                                                                                                                  |
| ---------------- | ----------- | -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `crit`           | SEV1・2     | サービス全体〜一部が停止。ユーザー影響が重大 | 即ページ（オンコールを起こす）。Slack等を併用する場合は `@channel` 等の全員メンションで即座に注意を引く。時間帯を問わず即応 |
| `warn`           | SEV3・4     | サービスの一部に影響／一時的な影響           | チケット・Slack通知（メンションなし）。営業時間内に対応                                                                     |
| `info`           | SEV5        | 修正は不要だが、報告・記録しておきたい       | 通知のみ（メンションなし、ページしない）。復旧対応は伴わない                                                                |

- **crit / warn はバーンレートと整合させる。** SLI/SLOアラートの「速い消費＝即ページ」が `crit`、「遅い消費＝チケット」が `warn` に対応する。レベルは新しい軸ではなく、既存の緊急度判断に名前を付けたもの。
- **`info` を格下げの受け皿にしない。** `info` は「修正は要らないが報告は要る」狭い帯だけに使う。報告すら要らない（誰も見なくてよい）ものは `info` アラームにせず、ダッシュボード／ログへ置く。ここを緩めると別のバケツでオオカミ少年が再発する。
- **通知の「強度」（メンション有無）もレベルに連動させる（具体は上表）。** 強度を揃えてしまうと、`crit` が `warn`/`info` の雑音に埋もれて見逃されるか、逆に軽微な通知にまで人を呼び出してオオカミ少年化するかのどちらかが起きる。
...
- **レベル**：各アラームに深刻度レベル（crit/warn/info）が付き、名前のサフィックスに表れているか。割り当てがユーザー影響の大きさ（SLI/SLOではバーンレート）と整合するか。info が「報告すら要らない」項目の受け皿になっていないか。通知の強度（メンション有無等）がレベルと連動しているか
