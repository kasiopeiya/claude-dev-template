...
|---|---|---|---|
| `crit` | SEV1・2 | サービス全体〜一部が停止。ユーザー影響が重大 | 即ページ（オンコールを起こす）。時間帯を問わず即応 |
| `warn` | SEV3・4 | サービスの一部に影響／一時的な影響 | チケット・Slack通知。営業時間内に対応 |
| `info` | SEV5 | 修正は不要だが、報告・記録しておきたい | 通知のみ（ページしない）。復旧対応は伴わない |
...
| レベル | 各アラームに深刻度レベル（crit/warn/info）が付き、名前のサフィックスに表れているか。割り当てがユーザー影響の大きさ（SLI/SLOではバーンレート）と整合するか。info が「報告すら要らない」項目の受け皿になっていないか |
