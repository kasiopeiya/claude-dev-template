...
- **SLI は必ず測定できる状態にする（計装）。** 「リクエスト成功率」を測るにはリクエスト数・エラー数の集計が要る。集計・メトリクス化の仕組みが無ければ SLI は絵に描いた餅になる。
- **SLI はダッシュボードで現在値が一目でわかるようにする（推奨）。**
- **SLO 逸脱（エラーバジェットの消費）はアラート候補。** ただしアラート化は本ポリシーの「症状ベース／actionable」に従わせる。全 SLI を即ページにすると「オオカミ少年」に逆戻りする。
...
| SLI/SLO由来 | 要件定義で決めた SLI/SLO がすべて監視項目になっているか。SLI は測定可能（計装済み）で、ダッシュボードで見えるか |
