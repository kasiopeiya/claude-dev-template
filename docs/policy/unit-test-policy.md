...
ユニットテストは**速い**ことと**決定的**であることが絶対条件である。共有依存でなくても、この2条件を壊す依存はテストダブルで置き換える。
...
<details>
<summary>なぜ速度と決定性が絶対条件なのか</summary>

- **速度：** ユニットテストは開発中に何度も繰り返し実行されることで価値を生む。1ケースでも遅いと「テストを流すのが面倒」になり、実行頻度が落ちてフィードバックループが壊れる。実行が遅い依存は置き換え、本物を使った検証は統合テストに委ねる。
- **決定性：** 同じコードなのに実行のたびに成否が変わる（flaky）と、失敗が「またいつものやつ」と無視されるようになる（オオカミ少年問題）。システム時刻や乱数のように外部要因で結果が変わる依存はテストダブルで固定し、入力を一定にする。
...
モックによる呼び出し検証は、テストを実装の詳細（どの依存をどう呼んだか）に結びつけやすく、退行（バグ）の検出能力も下がる。外部システムとの連携が「本当に動くか」は、本物（または本物に十分近いフェイク・契約）を使う統合テストで初めて保証できる。そのため外部通信の検証は統合テストに寄せ、単体テストでのモック検証は本物もフェイクも用意できない暫定策などに限定する。本物を使うか境界でダブル化するかは [test-strategy-policy.md](test-strategy-policy.md) に従う。
</details>
...
- **テストは「失敗したときに読まれる」もの：** テストが読まれる主な場面は失敗時であり、そこで素早く原因を把握できることが最重要である
...
なお、テストデータ生成のファクトリ（[テストデータの共通化パターン](#テストデータの共通化パターン)の Object Mother / Test Data Builder）は DAMP と矛盾しない。`createUser({ membershipLevel: 'gold' })` のように **入力データの意図が呼び出し側に残る**抽象化は説明性を高める。DAMP が避けたいのは、**Act（実行）や Assert（検証）のロジックそのものをヘルパーに隠してしまう**抽象化である。
