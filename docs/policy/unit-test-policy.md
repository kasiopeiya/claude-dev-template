...
テストダブル（dummy/stub/spy/mock/fake）の厳密な定義は [test-terms.md](test-terms.md) を参照（単一の真実）。ここでは単体テストで頻出する**スタブとモックの使い分け**だけを再掲する。
...
単体／統合の境界は「本物のプロセス外依存に実際に触れるか」で決まる（[test-terms.md](test-terms.md) 参照）。共有依存（外部API等）をテストダブルで差し替えれば本物に触れていないので、外部依存への**呼び出し自体の検証（モック）**も技術的には単体テストとして書ける。
