...
> - 単体テストでは原則モックを使用しない（呼び出し回数・順序が仕様である等、限定的な例外のみ。4節参照）。スタブも可能な限り避ける。
...
なお、テストデータ生成のファクトリ（6節の Object Mother / Test Data Builder）は DAMP と矛盾しない。`createUser({ membershipLevel: 'gold' })` のように **入力データの意図が呼び出し側に残る**抽象化は説明性を高める。DAMP が避けたいのは、**Act（実行）や Assert（検証）のロジックそのものをヘルパーに隠してしまう**抽象化である。
...
- カバレッジ計測で未検証の経路が見つかった場合、その経路がテスト対象（8節の基準）に該当するなら**テストケースを追加する**
