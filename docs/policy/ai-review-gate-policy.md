...
> - **レビューは最大3回（初回を含む）。毎回すべての重大度を直す（例外は本文参照）**
...
修正のたびに、重大度にかかわらず指摘をすべて直す（上限に達して不合格になった回を除く。下の「レビューは最大3回まで」参照）。ただし次の2種類だけは、重大度にかかわらず直さずに Issue にしてよい。**この2つ以外は、重大度にかかわらず直す。**
