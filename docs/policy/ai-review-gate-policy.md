...
下の「適用範囲」に挙げたレビューは既定で、**今回の差分が生んだ違反だけ**を判定する。変更していない箇所に前からある違反は探さず、指摘も起票もしない。全文を判定するのは、引数 `--full` を渡したときと、指定したパスに差分が無いときだけである。
...
この節は、レビューの後に修正役がいて直す場面に掛かる規定である。定期チェックのように修正役がおらず、指摘を Issue にして終える場面は、この節の対象外であり、「残してよい例外」の話ではない。
...
- **目的を見失えば、速く進むほど遠ざかる／割れ窓を放置しない**：既定のレビューの目的は、今回の変更を確かめることである。前からある違反を毎回のレビューに相乗りさせると、出力が増えて目的の方を損なう。前からある違反は、`--full` を渡したときに拾う
