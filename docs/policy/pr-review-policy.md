...
PR本文は前提条件に含めない。なぜ変えたかはコミットメッセージか Issue に残り、レビュワーはそこから辿れるためである。AI が push したときは必ず `/pr-body` で付ける。人間が push したときは、マージで Issue を閉じたいときなど本文が要るときだけ付ける。
...
ただし、分割が常に最善とは限らない。密に関連する変更を無理に切り離すと、各PRが単体では意味をなさず文脈を欠き、かえってレビューしづらくなることがある。まとめて出した方が適切だと判断する場合は、大きいまま出してよい。
