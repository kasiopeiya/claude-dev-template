...
AIエージェントが、PRを出す・レビューするときに、コメントの書き方・マージ判断・PRのサイズ感について迷ったときに参照する指針。レビューで**何に目を向けるか（観点）**——インターフェース・コメント・名前の適切さ——は [code-review-guide](../guide/code-review-guide.md) の「Part B：レビューの観点」を参照。
...
レビューを通じて、実装の背景・設計の意図・言語やフレームワークの慣用表現が自然にチーム内へ広まる。ドキュメントに書かれない暗黙知を伝える場として機能する。

### 他者の目が入ると思い込みや偏りが是正される

レビューがなければ、コードのスタイルや設計は自然と個人の習慣に収束する。他者の目が入ることで、自分では気づかない思い込みや偏りが是正される。「誰かに見せる」という前提があるだけで、コードを書く側の意識も変わる。
