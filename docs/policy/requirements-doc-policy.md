...
**理由**：畳んで隠すのは、詳細が要るのは疑問を持った読み手だけで、表を追うだけの読み手には邪魔になるからである（documentation-policy「トグル記法を積極的に活用する」）。要件定義書は凍結する文書なので、隠した内容が更新されずに腐る心配もない。潰れた表は、読み手がこの文書だけを頼りに全体像を掴むのを妨げる（refined-engineer-judgment-principles の「理解は委譲できない」——理解を育てる形で渡す責務は AI にある）。
...
**この表は、documentation-policy が継承先に委ねた「7列以上を許す表」の名指しでもある。** 要件定義書で7列以上を許すのはこの表だけで、1セル80文字の上限はこの表にも効く。
...
| 表は幅で潰さない                   | 「どの表のどの項目を詳細へ出すか」が名指しする項目が列として残っている／詳細が `<details>` ではなく見出し（`#### BR-xx`）で書かれている／`<details>` の直後・`</details>` の直前に空行が無い／追跡列が詳細トグルへ移されている／属性の値制約一覧に `根拠`・`理由` 等の列が足されている／documentation-policy「表は俯瞰のためにある」が定める列数・セル文字数の上限を超えている（7列以上を許すのは属性の値制約一覧だけ） |
