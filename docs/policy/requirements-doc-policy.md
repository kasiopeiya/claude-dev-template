...
**方針**：表を俯瞰の道具に保つ規定（列数・セル文字数の上限、詳細を表の直下へ出すこと）は documentation-policy-for-humans「表は俯瞰のためにある」と Rule document-writing-for-humans.md「表のセル文字数の上限」に従う。要件定義書はそこに2つを上乗せする——**詳細の置き場を詳細トグル（`<details>`）に固定する**ことと、**表から出す項目を表ごとに名指しする**ことである。
...
- **表は幅で潰さない（セル文字数）**：Rule document-writing-for-humans.md「表のセル文字数の上限」が定める上限を超えている
