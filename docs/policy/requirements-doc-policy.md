...
**方針**：**表は索引に徹する。** 1行の説明・理由・列挙は表に置かず、表の直下の**詳細トグル**（`<details>`）に置く。

**理由**：表は列が増えるほど1列の幅が狭くなる。列が10を超えると、短い列は1文字ずつの縦書きに潰れ、長い列は1行だけで画面を埋める。潰れた表は、読み手がこの文書だけを頼りに全体像を掴むのを妨げる（refined-engineer-judgment-principles の「理解は委譲できない」——理解を育てる形で渡す責務は AI にある）。

畳んで隠すのは、詳細が要るのは疑問を持った読み手だけで、表を追うだけの読み手には邪魔になるからである（documentation-policy「トグル記法を積極的に活用する」）。要件定義書は凍結する文書なので、隠した内容が更新されずに腐る心配もない。
...
7列以上を厳しくするのは、**そこが「列を削るべき表」だからである。** 列が7つ以上あってセルも長いなら、上限に収める前にまず列を減らせないかを検討する（上表の名指しに追加できないか、`出所` のように値を縛れないか）。上限は、削れなかったときの最後の歯止めとして働く。

この上限は歯止めであって、可読性を保つ主役ではない。表を読める状態に保つのは、上表が名指しする項目を詳細トグルへ出すことと、`出所` に書ける値を縛ること（「出所ラベル」節）である。上限は、そこで取りこぼした桁違いのセルを拾う。
...
| 行に分けられない（1つの項目の必須要素が並ぶ） | 表から外し、詳細トグルへ移す |
...
| 表は幅で潰さない                   | 「どの表のどの項目を詳細へ出すか」が名指しする項目が列として残っている／詳細が `<details>` ではなく見出し（`#### BR-xx`）で書かれている／`<details>` の直後・`</details>` の直前に空行が無い／追跡列が詳細トグルへ移されている／属性の値制約一覧に `根拠`・`理由` 等の列が足されている／文字数上限（5〜6列は150文字・7列以上は80文字）を超えるセルがある |
...
| ビジネスルール一覧（不在）         | 入出力情報一覧の `内容` にある属性に、値制約を定めるルールが無い／情報どうしの関係に多重度を定めるルールが無い                                                                                                                                                                                                                                           |
| 入出力情報一覧                     | 送り手／受け手が無く主体が読み取れない／ID が無く参照できない／テーブル名・カラム等の物理名に降りている／`内容` が「〜情報」の粒度で止まり属性名まで列挙されていない                                                                                                                                                                                     |
...
| 機能要件の可視化（必須図）         | 機能要件のサブ項目のうち区分が「必須」の図が欠けている／分岐・ループを持つ機能にアクティビティ図が無い／機能ごとのアクティビティ図が詳細トグルに畳まれず見出し（`####`）で並べられている                                                                                                                                                                 |
