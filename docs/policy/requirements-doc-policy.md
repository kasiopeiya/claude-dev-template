...
| 表は幅で潰さない                   | 「どの表のどの項目を詳細へ出すか」が名指しする項目が列として残っている／詳細が `<details>` ではなく見出し（`#### BR-xx`）で書かれている／`<details>` の直後・`</details>` の直前に空行が無い／追跡列が詳細トグルへ移されている／属性の値制約一覧に `根拠`・`理由` 等の列が足されている／documentation-policy「表は俯瞰のためにある」が定めるセル文字数の上限を超えている／7列以上の表で列を減らす手を試していない |
