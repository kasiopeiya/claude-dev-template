...
- [記述すべきは「ビジネスの基準（What）」であり、「実装の手段（How）」ではない](#記述すべきはビジネスの基準whatであり実装の手段howではない)
- [変動リスクのある「実現コスト」や「技術予測」は書かない](#変動リスクのある実現コストや技術予測は書かない)
- [顧客の「要望」を「測定可能な前提条件」に翻訳して明記する](#顧客の要望を測定可能な前提条件に翻訳して明記する)
- [出所ラベル：入力資料にない記述、および入力資料からの削減・変更には出所を明示する](#出所ラベル入力資料にない記述および入力資料からの削減変更には出所を明示する)
- [メンテナンス性より理解しやすさを優先する](#メンテナンス性より理解しやすさを優先する)
- [表は幅で潰さない](#表は幅で潰さない)
- [要件定義書の構成（必須項目と記述基準）](#要件定義書の構成必須項目と記述基準)
...
**方針**：5列以上の表は、1セルを200文字以内に収める。

**理由**：表は列が増えるほど1列の幅が狭くなる。狭い列に長い文章が入ると、そのセルだけ縦に伸びる。潰れた表は、読み手がこの文書だけを頼りに全体像を掴むのを妨げる（refined-engineer-judgment-principles の「理解は委譲できない」——理解を育てる形で渡す責務は AI にある）。
...
数えるのは**文字数**である（半角・全角を区別せず1文字と数える）。

この上限は歯止めであって、可読性を保つ主役ではない。表を読める状態に保つのは、長い本文を見出しへ移すこと（下記）と、`出所` に書ける値を縛ること（「出所ラベル」節）である。上限は、そこで取りこぼした桁違いのセルを拾う。

上限を超えたら、**超えた理由で直し方を分ける。** 問いは1つ——そのセルは行に分けられるか。

| セルの状態                                    | 直し方                         | 例                                |
| --------------------------------------------- | ------------------------------ | --------------------------------- |
| 行に分けられる（2つ以上の項目が入っている）   | 行を分ける                     | 機能仕様の `主要な例外`           |
| 行に分けられない（1つの項目の必須要素が並ぶ） | 表から外し、見出し＋本文へ移す | ビジネスルール一覧の `ルール内容` |

見出しへ移すときは、その行の `ID` と名前を見出しにする（例：`#### BR-01 付与率`）。表には `ID` が残るので、読み手は `ID` から本文を引ける。
...
版数・作成者・承認ステータスは本文に持たない。git 履歴と Issue で追跡する（documentation-policy「経過・状態の記述をしない」）。
...
- 対話の経緯は本文に持ち込まない（documentation-policy「経過・状態の記述をしない」）。要件そのものに紐づく根拠——制約を再判定した理由など——だけを「制約と前提」の判定理由に残す。
...
- **ルール本文は表に置かない。** 表は索引として ID・名前・タイプ・関連・出所だけを持ち、本文は `#### BR-01 付与率` の形で1ルール1見出しを立てて書く（「表は幅で潰さない」）。本文は表の直後に、表と同じ ID 順で並べる。
...
**あるべきルールの不在の判定**：書かれたルールの不備と違い、**ルールが無いことは読んでも目に入らない**。書き手の「重要そうか」に任せず、[入出力情報一覧](#入出力情報一覧) の `内容` 列から次を1つずつ拾い、対応するルールを引く。無ければ違反。

| `内容` 列から拾うもの     | 引くルール       | 引く先             |
| ------------------------- | ---------------- | ------------------ |
...
入出力情報一覧の `内容` 列に現れる属性1つにつき1行を立て、その値制約を書く。
...
- **制約の根拠（なぜその値なのか）は、表の直後のトグルブロックに `ID` / `根拠` の2列表で書く。表に列を足してはならない。** 根拠は全属性に要らない。列の記述だけでは読み取れないものにだけ付ける。

根拠に場所を用意するのは、**用意しないと既存の列へ押し込まれる**からである。`取りうる値` に「〜だからこの上限にした」と理由が混ざると、列が横に伸びて表が読めなくなる。トグルに隠すのは、根拠が要るのは疑問を持った読み手だけで、表を追うだけの読み手には邪魔になるため（documentation-policy「トグル記法を積極的に活用する」）。

なぜビジネスルール一覧と分けるか：この表の行数は**属性の数に比例して機械的に増える**（実案件では数百行）。判断・計算のルールと同居させると、人が読むべきルールが定型行の海に埋もれる。増え方の違う2つを1つの表に置かない。

#### 入出力情報一覧

- 業務が扱う情報を列挙する。列：`ID`（`IO-01` 形式）/ `情報名` / `入出力区分` / `送り手` / `受け手` / `内容` / `取扱量` / `頻度` / `利用目的` / `利用業務` / `出所`（「出所ラベル」節）。`備考` は置かない——出所ラベルは `出所` 列が持ち、ほかに書くべきことは各列が持つ。
...
- **`内容` 列は属性名まで列挙する。**「購入情報」のような括りで止めない。ここが属性の唯一の入り口なので、粗く書くとビジネスルールの不在を誰も検知できなくなる。
...
- **例外は列ごとに適用する。** 読める列は数値で書き、読めない列だけ発生契機にする（例：利用者の任意操作は件数が読めるので取扱量は数値、周期は読めないので頻度は `随時`）。両方の列をまとめて例外にすると、読める情報まで落として要件の測定可能性を下げる。
...
- 列：`機能ID` / `機能名` / `対象業務` / `優先度` / `優先度の理由`（「優先順位」節）。
...
- 列：`機能ID` / `アクター` / `トリガー`（ユーザー・他システム・スケジューラ）/ `入力` / `適用ルール`（その機能が従うビジネスルール）/ `出力` / `事後条件` / `主要な例外`。
...
| 出所ラベル                         | ラベルが無く、入力資料からの削減か作成者の補完かが区別できない／`出所` 列に「出所ラベル」節が定める値以外が書かれている（合意相手・経緯・理由の書き足しを含む）                                                                                                                         |
| 表は幅で潰さない                   | 5列以上の表に200文字を超えるセルがある／ビジネスルール一覧に `ルール内容` の列が残っている／表から外したルール本文が `#### BR-xx ルール名` の見出しになっていない／属性の値制約一覧に `根拠`・`理由` 等の列が足されている（根拠は表の下のトグルへ）                                     |
...
| ビジネスルール一覧                 | 「適切に計算する」等、計算式・境界値が曖昧なまま／分類・区分を定めるルールに多重度・排他性が無い／関係（AがBを持つ）を定めるルールに多重度が無い／属性の値制約を定めるルールに必須/任意が無い／`行動` のルールに主体・条件・義務/禁止のいずれかが無い／タイプ（行動・定義づけ）が未分類 |
| ビジネスルール一覧（不在）         | 入出力情報一覧の `内容` 列にある属性に、値制約を定めるルールが無い／情報どうしの関係に多重度を定めるルールが無い                                                                                                                                                                        |
| 入出力情報一覧                     | 送り手／受け手が無く主体が読み取れない／ID が無く参照できない／テーブル名・カラム等の物理名に降りている／`内容` 列が「〜情報」の粒度で止まり属性名まで列挙されていない                                                                                                                  |
| 業務フロー図（描く場合）           | システム内部の処理フロー・コンポーネント間シーケンス・画面遷移が描かれている（How）                                                                                                                                                                                                     |
| トレーサビリティ                   | 追跡列が無く各表が孤立している／列で追える紐づけを独立した相互参照表に再掲している／機能仕様の `入力`・`出力` 列に、入出力情報一覧の `ID` として実在しない値が書かれている                                                                                                              |
...
| 非機能要件（網羅・SLI）            | カタログの分類の一部が節として立たず黙って欠落／非機能が数行しかない／CPU・キュー長等の内部指標が SLI に混入／計算式のカウント・タイム種別が未確定                                                                                                                                      |
...
| 受入基準                           | `検証対象` が空、または許容値（機能ID／業務名／SLO／非機能の分類名）以外が書かれている／事後条件・SLO 目標値を再掲している／Must・Should 機能の一部が欠けている                                                                                                                         |
| リスクと対応（悪用インセンティブ） | 収益・課金に直結する仕組みなのに、不正利用・水増しのリスクが記載されていない                                                                                                                                                                                                            |
| オープンクエスチョン（出口）       | 要件段階で決めるべき事項が書かれている（名指し例外の SLO 目標値を除く）／解決の相手が書けない／解消期限が無い／期限が工程でなく日付で書かれている／その場で聞けば済む事項が OQ 化されている                                                                                             |
