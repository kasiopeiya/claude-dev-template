...
ラベルは、補完・削減・変更が起きた表に `出所` 列として持たせる。ラベルが不要な行には `—` を置く（空欄にすると書き忘れと区別できない）。ラベルは上記2種のみとし、表記を増やさない（`提案・叩き台` は未合意を意味するので「要合意」等を重ねない）。
...
- 列：`ID` / `ルール名` / `ルール内容（具体値まで）` / `タイプ` / `関連ルール` / `出所`（「出所ラベル」節）。
- タイプは `行動`（人の行動を義務・禁止・条件づける）と `定義づけ`（分類方法・計算式を定める）に限る。
- タイプごとに書く項目が決まっている。下表の項目が1つでも欠けていたら未完成とする。
