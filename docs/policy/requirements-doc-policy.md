...
- 図が要る項目（エコシステムマップ・入出力情報の IPO 図・業務フロー図・機能仕様のアクティビティ図・状態遷移）は `/design-doc-mermaid` で作成する。
...
- 計算式・境界値は具体値まで書く。そのままテスト・バリデーションの種になるため。
- 属性の値制約（取りうる値・必須/任意・桁・形式）も `定義づけ` としてここに書く。ただし業務・外部が課す制約だけを書き、カラム幅などの実装都合は書かない（設計側の責務）。
...
- 属性の値制約（桁・型・取りうる値等）はカラム仕様として書かず、ビジネスルール一覧（`定義づけ`）へ回す。
...
- 重要な一部だけを抜き出さない。抜き出す基準は主観的でモレを生むため。機能ごとに変わるのは「図を添えるか否か」だけ。
- 各機能に書くこと：アクター／トリガー（ユーザー・他システム・スケジューラ）・入力・処理で適用するビジネスルール・出力・事後条件・主要な例外。
- **「何を達成するか」を論理レベルで書き、画面・操作手順・帳票レイアウトは書かない**（How）。
- 入力／出力の中身は入出力情報一覧、判断／計算はビジネスルール一覧を ID で参照する（二重管理しない）。
- 画面のないシステム（バッチ・API）も同じ型で書ける（アクター＝スケジューラ／他システム、出力＝ファイル／レスポンス）。
- **基本は表で書く**。機能一覧との対応・ID 参照を担うため。
- 表で追えない構造（分岐・代替フロー・ループ・並行・複数アクターの相互作用）を持つ機能は、逆変換テスト（表・箇条書きに戻して情報が落ちるなら図）で要否を判断し、**アクティビティ図（フロー）の併記を推奨**する。
- ユースケース図は不要。シーケンス図を使う場合も論理アクター間に留め、システム内部のコンポーネント間に降りない（What/How の切り分け）。
...
- 未決事項を書く（無ければ「なし」と明記する）。
- 各項目に**解決の相手**（誰に聞けば決まるか：顧客・外部の関係者・社内の意思決定者 等）と**解消期限**（`要件凍結前` ／ `該当機能の設計前` ／ `リリース前`）を列に持たせ、出口の無い未決事項を残さない。
- 解消期限が最も重要な列である。**その未決事項が要件段階で決めるべきものか、設計以降に送ってよいものかを、読み手が一目で判別できる**必要があるため。
...
| ビジネスルール一覧                 | 「適切に計算する」等、計算式・境界値が曖昧なまま／タイプ（行動・定義づけ）が未分類                                                          |
...
| 機能仕様の可読性                   | 分岐・ループを含む機能が表だけで書かれ、逆変換テストで図にすべきものが図になっていない                                                      |
...
| オープンクエスチョン（出口）       | 解決の相手が書けない／解消期限が無い／期限が工程でなく日付で書かれている／その場で聞けば済む事項が OQ 化されている                          |
| 網羅性（要求の全数仕分け）         | 入力資料にある記述が要件定義書のどこにも反映されておらず、対応関係が追えない                                                                |
| 二重管理（SSOT）                   | 同じ事実（外部連携の関係・境界、データ量・頻度、SLO 目標値、事後条件）が複数箇所に詳細記載されている                                        |
