...
> **TL;DR（このポリシーの決定事項）**
>
> - **置き場所は抽象度で決める。** 実装の書き方なら rules、それ以外の本質寄りなら Policy、機械が合否を出せるならガードレール。Policy はプログラミング言語に依存させない（別言語のプロジェクトへコピーしてそのまま使えることが条件）
...
行為で発火する Policy に `applies-to` が無いのは付け忘れではない。読ませる責任は開発フローの入口（AI への基本指示・各 Skill）が持つ。逆に、パスで言えるのに無ければそれは付け忘れである。
