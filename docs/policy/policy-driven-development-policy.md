...
> - **Rule は既定で `paths` による標準ロードで届ける。** ただし `paths` を名指しのパス（`docs/policy/**`・`docs/` 直下のハブ）に掛けない。名指しのパスに効かせたい Rule は、Policy と同じ `hook.applies-to` を書き、編集時にだけ注入する
...
| **Rule**（法律・`.claude/rules/`）               | 書き方をそろえ、レビューで直す手間をなくす                         | 選んだ後の How。機械で判定できるものはガードレールへ移す（数値上限と直し方は残す） | 対象ファイルを読むと読み込まれる。無いときは hooks が指し示す。名指しのパスには `hook.applies-to` を使い、hooks が編集時にだけ指し示す | AAA の Act は必ず1行／見出しに連番を振らない                     |
...
3. 選んだ後の「どう書くか・どう作るか」か → 発火点をファイルパスで言えれば Rule（名指しのパスは `paths` でなく `hook.applies-to` に書く）、行為で発火するなら Skill
...
flowchart TD
    ...
    Q9 -->|Yes| Rule["📄 Rule<br/>paths を付ける（名指しのパスは hook.applies-to）"]
...
## 禁止・限定は、根拠を名指しできるときだけ書く

「なぜ」と同じく、**規定そのもの**も書いている最中に作文できてしまう。指示が触れていない論点を「〜だけ」「〜してはならない」で埋めると、その禁止は理由を誰も知らないまま後続のセッションを縛る。後で本当に必要になったとき、自分の作文と戦うことになる。空けておけば、そのときの材料で決められる（原則集「決定を遅らせる」）。

> [!IMPORTANT]
> **（AI・必須）** Policy に禁止・限定（「〜だけ」「〜に限る」「〜してはならない」「〜は〜しない」）を書く前に、その根拠を次のどれかから名指しする。名指しできなければ書かない。名指しした根拠は、後続のセッションが作文でないと確かめられるよう、規定のそばに書き添える。
>
> - **起点の Issue の本文**（対応方針・タスク一覧・今回作らないこと）：Issue 番号を書く
> - **人間との対話での指示**：「人間の指示」と明記し、人間が述べた理由を書く
> - **既存の Policy（原則集を含む）の規定**：文書名と節名を書く
> - **移し元の規定**（CLAUDE.md・Rule・他の Policy から移すとき）：移し元の文書名と節名を書く
> - **確かめた事実**（公式ドキュメント・実測など）：出典を書く

- ❌ 「レビューの判定範囲を既定で差分に絞る」Issue で、「`--full` を渡すのは人間だけ」と書き足す（Issue は誰が渡すかに触れていない） → ✅ 既定を差分にし、`--full` で全文に戻せることだけを書く
- ❌ 「`@path` の import は使わない」とだけ書く → ✅ 「`@path` の import は使わない。公式ドキュメントは `.claude/rules/` の import を読み込むと書いており、本体が丸ごと読み込まれる」と、確かめた事実の出典を添える
...
| コマンドで言える     | hook（後述の役割に限る）                | PR を作る・本文を上書きする        |
...
Rule も読まれて初めて効く。届き方は frontmatter で決まり、経路が2つある。

| Rule の frontmatter                    | いつ書くか                                                               | 届き方                                                                                                                                                    |
| -------------------------------------- | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `paths`                                | 名指しのパス（`docs/policy/**`・`docs/` 直下のハブ）以外に効かせたいとき | 対象ファイルを Read した時点で読み込まれる（標準ロード）。対象ファイルがまだ無い（新規作成）ときだけ `.claude/hooks/policy-loader.mjs` が代わりに指し示す |
| `hook.applies-to`（Policy と同じキー） | 名指しのパスに効かせたいとき                                             | `policy-loader.mjs` が編集時に指し示す。対象ファイルの有無を問わず、Policy の `hook.applies-to` と同じ扱い                                                |

`paths` には名指しのパス以外だけを列挙し、名指しのパスは `hook.applies-to` に書く。両方に同じパスを書かない。否定 glob は使えないため、除外は「名指しのパスを `paths` から外す」以外の方法を持たない。

新規ファイルだけ `paths` を policy-loader が肩代わりするのは、既存ファイルは Edit・Write の前に Read しており標準ロードが既に効いているため。二重に指すと文脈を無駄に食う。`hook.applies-to` は対象ファイルの有無にかかわらず毎回指し示す（読むだけのセッションでも編集前に確認させたいため）。

名指しのパスを分けるのは、読むだけのセッションが多いからである。過去176セッションの実測で、`docs/policy/` を読んだ35セッションのうち24（69%）は編集していなかった。`docs/design/` では14のうち2だった。

`.claude/rules/` に `paths` も `hook.applies-to` も無く置くと、毎回全文が読み込まれ CLAUDE.md と同じ扱いになる。置かない。常に効かせたいなら CLAUDE.md に書く（文字数上限を回避する抜け道になるため）。
...
| **ターン末ゲート** | 既にあるゲートを、AI がターンを終える前に走らせる              | 後段に着くのはターンが終わった後。AI が自力で直せる最後の機会がターン末にしか無い。1ターンの中で push まで終えるフロー（`/auto-dev`・`/auto-fix-ci`）では、その最後の機会が push の直前に移る（下記の git hook） |

> [!IMPORTANT]
> **（AI・必須）** **ターン末ゲートは1本に統合する。** 検査を足したくなったら hook を増やさず、その1本が呼ぶゲートを増やす。hook が増えない形なら、何個足しても代償は増えない（「Less is more」）。

### git hook は、コミット・push の瞬間に npm script を走らせる2本に限る

git hook（`.githooks/`）は Claude Code の hook と別物で、コミット・push の瞬間に走る。置いてよいのは次の2本だけである。

| git hook       | 何をするか                                                 |
| -------------- | ---------------------------------------------------------- |
| **pre-commit** | ステージ済みファイルへ機械的に掛ける整形と、生成物の再生成 |
| **pre-push**   | push の直前に、既存のゲートを走らせる                      |

pre-push は、1ターンで push まで終えるフローでターン末ゲートの役割を引き継ぐ位置にある。ターン末ゲートは未コミットの変更だけを見るので、push 済みのターン末には何も検査しないためである。ただし、いま走らせているのは整形チェック（`format:check`）だけで、ターン末ゲートが変更パスに応じて選ぶ他のゲートは走らない。

どちらも検査の中身は npm script が持ち、git hook が持つのは「いつ走らせるか」だけにする（「ターン末ゲートは1本に統合する」と同じ理由）。検査を足したいときは git hook を増やさず、npm script の側に足す。
