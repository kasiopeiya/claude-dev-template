...
- **起点Issue はモデルを最大にして作る**：使用可能な最も性能の良いモデルを選び、effort も最大にして実行する。ここでの切り分けが以降のすべてのIssueの土台になり、誤ると作った Issue が丸ごと作り直しになるからである。フェーズごとのモデルの使い分けは[開発フローガイド](development-flow.md)にある。
...
**着手の段を分析し、段番号をタイトルの先頭に入れる**（例：`2. F-02 ポイント利用`）。Issueを開かないと順番が分からない状態にしないためである。**番号は親にするフェーズごとに1から振る**——振り方の規約は[Issueの階層ガイド](../reference/issue-hierarchy.md)が正典。

フェーズをまたぐ前後（開発準備が先、設計・実装が後）はフェーズそのものが表すので、ここで決めるのはフェーズ内の順序だけである。次の順に置き、番号は各Issueの `## ブロッカー` 節から計算する。

- **要件定義**：オープンクエスチョン（互いに依存しないので全件が段1）
- **開発準備**：全体構造設計 → 足場
- **設計・実装**：後から入れるとやり直しになる非機能 → 曳光弾 → 前の段の機能が作る情報を入力にする機能

**機能どうしの前後は、機能仕様の入力・出力（入出力情報一覧のID）を突き合わせて判定する。入力（内部）区分は、それを生む機能まで辿る。** 残りの非機能・スタブ追跡・リスクは、関わる機能と同じ段に置く。

**設計・実装でいちばん早い非機能だけ、依存ではなくやり直しコストで順序を決める。** 暗号化・認証認可・データの所在は関わる機能が全機能に及ぶので、依存で決めると最後の段に落ちる。だが後から入れるとデータ移行と権限設計をやり直すことになるため、曳光弾より先に置く。
...
**起点Issueを sub-issue に割ると、起点Issueは親になり、段番号は子の段の範囲に変わる。** 番号は着手単位＝子の側へ移る。振り直しは `/issue-deps` が行う。
...
作った起点Issueは、Issueの階層ガイドの「親の決め方」に従って上表のフェーズIssueにぶら下げ、以降は開発フローガイドに沿って**各Issue単位**で回す。
...
```text
開発準備（フェーズIssue）
└ 1〜4. 全体構造設計（起点Issue・親なので子の段の範囲）
  ├ 1. 外部インターフェース仕様
  ├ 1. DB概念モデル
  ├ 2. アプリアーキテクチャ１：配置のトポロジー
  ├ 3. アプリアーキテクチャ２：コードの内部構造
  ├ 3. インフラ構成図
  └ 4. ディレクトリ構成方針
```
...
| 1   | DB概念モデル（エンティティ・関係・一意性の業務ルール） | 描く   | なし                               | `/design`                   |
| 2   | アプリアーキテクチャ１：配置のトポロジー               | 決める | DB概念モデル                       | `/grill-me` → `/create-adr` |
| 3   | アプリアーキテクチャ２：コードの内部構造               | 決める | 配置のトポロジー                   | `/grill-me` → `/create-adr` |
| 3   | インフラ構成図                                         | 描く   | 配置のトポロジー                   | `/design`                   |
| 4   | ディレクトリ構成方針                                   | 決める | 配置のトポロジー・コードの内部構造 | `/create-adr`               |

**配置のトポロジー**＝デプロイ単位と通信方式（モノリス／モジュラーモノリス／サービスベース／マイクロサービス／イベント駆動）。**コードの内部構造**＝レイヤーと依存方向（レイヤード／クリーンアーキテクチャ／Ports & Adapters）。この2つの選び方は [architecture-selection-guide](../reference/architecture-selection-guide.md)、ディレクトリ構成方針の絞り込みは [directory-structure-guide](../reference/directory-structure-guide.md) が持つ。

**性格**は、要件定義書とすでに決まった事項から導ける項目を「描く」、新たに選ぶ項目を「決める」と呼び分けている。決めるものは却下案ごと ADR に残す。

**sub-issue のタイトルの先頭に上表の段番号を入れ、ブロッカー欄も埋める**（例：`2. アプリアーキテクチャ１：配置のトポロジー`）。起点Issueに段番号を入れるのと同じ理由——Issueを開かないと順番が分からない状態にしない——がそのまま当てはまる。
