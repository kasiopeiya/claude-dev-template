...
**着手の段を分析し、段番号をタイトルの先頭に入れる**（例：`2. F-02 ポイント利用`）。Issueを開かないと順番が分からない状態にしないためである。**番号は親にするフェーズごとに1から振る**——振り方の規約は [Issueの階層ガイド](../reference/issue-hierarchy.md#段番号着手順) が正典。
...
作った起点Issueは、Issueの階層ガイドの「親の決め方」に従って上表のフェーズIssueにぶら下げ、以降は [開発フローガイド](development-flow.md) に沿って**各Issue単位**で回す。
