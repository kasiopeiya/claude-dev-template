...
9. **push・CI（人間/AI→CI自動）**：リモートブランチに push すると CI が検査を回し、PR を自動生成する。AI の範囲はトピックブランチの作成と push までで、`gh pr create` は使わない（hook が拒否する）。マージで Issue を閉じたいときなど、本文が要るときは push 後に `/pr-body <Issue番号>` で PR のタイトルと本文を差し替える。dev への cdk deploy と結合テストは main へマージした後に自動実行される
