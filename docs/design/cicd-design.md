...
| 脆弱性で落とす閾値を設け、それ未満は落とさない                                                                         | どこまでを受容するかは判断であり、機械に委ねてよいのは正解が一意に決まる作業だけ（cicd-policy）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| AI は **advisory**（マージ可否の required check には含めない）                                                         | AI の判断は非決定的（同じ差分でも出力が揺れうる）。cicd-gate の芯は決定論——required check は一意に決まる作業の結果でなければならない（cicd-policy）。AI を cicd-gate に入れると、Claude の障害やブレでマージが止まる                                                                                                                                                                                                                                                                                                                                                                                        |
