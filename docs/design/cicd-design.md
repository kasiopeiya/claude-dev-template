...
## 基本思想：トランクベース開発で高速に開発できるようにする

基本思想はトランクベース開発。**小さい変更を頻繁に main へ入れられること**を重視し、それを実現できるCI/CDパイプラインを設計した。  
具体的には主に４つの設計ポイントがあった。

①実環境へのデプロイをどのタイミングでやるのか  
②どうやってCIを充実させつつ、実行時間を短くするのか  
③コーディングエージェント開発でのレビューボトルネック問題にどう対応するのか  
④高速開発と安全性をどうやって両立するのか

### ①実環境へのデプロイをどのタイミングでやるのか

最も時間がかかるAWS 環境にデプロイする工程——deploy と結合テスト——は、main へマージした後に実行している。これにより、mainにマージするまでの開発サイクルを大幅に短縮できる。  
この方式のデメリットとして、デプロイしていない状態でPRをレビューすることになる問題がある。実際にCDK（CloudFormation）ではデプロイ時に発覚するエラーが多い（IAM の制約・リソース名の衝突・サービス上限など）。  
そのため、未デプロイのCDKコードレビューは、やり直しのリスクがある。しかし今回はこれを制約として許容し、それよりも開発スピードを重視する選択をした。  
mainマージ後にデプロイエラーになったとしても、修正のPRを即時に出すことができるので問題ないと考えている。

### ②どうやってCIを充実させつつ、実行時間を短くするのか

jobの並列実行と変更種別判定（変更対象のファイル種別に応じたjob実行）により、可能な限りの両者の両立を図った。  
AI駆動開発において静的解析やテストを充実させ、機械的な仕組みで品質を担保することは重要であるが、一方でこれらの増加は開発スピードを重視するトランクベース開発の思想と衝突してしまう。  
そのため、jobの並列実行と変更したファイル種別により実行条件を判定する（例：docsのみ変更など）ことで対処した。

### ③コーディングエージェント開発でのレビューボトルネック問題にどう対応するのか

コーディングエージェントを使ったチーム開発では、レビュー者に負担が集中する。AIは大量の変更を高速にできるが、AI成果物の認知負荷は高く、レビューがボトルネックになり開発のスピードが出ないという問題が生じる。  
我々はこの問題に対応するために、２つの対応を実施している。

１つ目は、「PRの変更行数上限を定める」  
PRあたりの変更行数上限を定めて、それを超える場合はレビューを拒否できる運用とした。  
変更行数の多いPRレビューは人間にとって認知負荷が高く、またPRが大きいほどレビュー精度も落ちてしまい、かつ時間もかかってしまう。

２つ目は、「人間がレビューするべきものとそうではないものを仕分ける」  
コーディングエージェントでの開発が前提の時代に、全てのコードをレビューするのは現実的ではない。  
そのためレビュイーの力量や変更されたコードの重要さにより、人間がレビューするべきPRと、AIレビューのみで通過させるPRを仕分けることとした。

これらを実施するためにAIを使用した判定を行った。なお、これらの判定の方針は[PRレビューポリシー](./docs/policy/pr-review-policy.md)に準拠したものである。

### ④高速開発と安全性をどうやって両立するのか

安全に高速開発するためにはヒューマンエラーを防止するための仕組み作りが必要である。そのため人間が手動で操作する工程を可能な限り排除した。  
例えばPRの作成自動化によりマージ先指定ミスを防止、status checkによりCIエラーのコードをマージできないようにしている。
