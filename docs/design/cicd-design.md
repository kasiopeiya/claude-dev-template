...
| 実装不在時の休眠     | 変更検知が `^app/`・`^infra/` のため、`app/`・`infra/` が空の間は `ci-app`・`ci-cdk`・`cdk-deploy` が常にスキップされる。参照実装（`samples/`）の型検査・単体テスト・アーキテクチャテスト・CDK スナップショットは CI では走らず、手元の `npm run check:static` と各ワークスペースの `npm run test` が受け持つ。`ci-common`（lint・knip・依存監査）だけは `samples/` を見るため常時有効 |
