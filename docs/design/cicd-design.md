...
| --------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| **Dependabot が見るのは GitHub Actions だけ**（npm を足さない）                               | GitHub は Dependabot の実行に secrets を渡さない。npm を足すと更新 PR が deploy を起動し、AWS 認証が落ちてゲートが赤くなる——**その PR は永久にマージできない** | Dependabot 用の secrets の設定か、deploy を起動する条件の見直し |
