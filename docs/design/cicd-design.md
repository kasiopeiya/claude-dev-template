...
このリポジトリの CI/CD を変更・レビューする開発者／AIが、**なぜこの構成なのか**を知りたいときに参照する。  
実行される処理・条件・順序は `.github/` 配下の定義が SSOT であり、本書には転記しない。

> [!IMPORTANT]
> **TL;DR（この設計の決定事項）**
>
> - **マージまでの経路には速く答えが出るものだけを置く**。実 AWS 環境に触れる deploy と結合テストは main へマージした後に行う
...
## 設計のポイント：トランクベース開発で高速に開発できるようにする

このパイプラインが最適化するのは、**小さい変更を頻繁に main へ入れられること**である。トランクベース開発はそれで成り立つ。1回あたりの待ち時間が長いほど、変更を小さく刻む動機が削られ、まとめて大きく入れる方へ流れる。

だから、マージまでの経路には**速く答えが出るものだけを置く**。実 AWS 環境に触れる工程——deploy と結合テスト——は、main へマージした後に実行する。deploy はパイプラインで最も時間がかかるうえ、dev は1環境しかないのでブランチ間で奪い合いになる。AWS 側の一時障害でマージが止まることもない。

この設計の他の判断——検査を並行に走らせること・required check を cicd-gate 1つにすること・dev へ入る経路を main だけにすること・cdk diff をゲートに繋がないこと——は、**すべてここから導かれている**。迷ったときは「その工程はマージを遅らせるか、止めうるか」で判断する。
...
赤い破線の枠は、**中身を持たないもの**（「スコープ外」参照）。黄色の枠は、**required check（マージ可否）とは別系統の advisory 処理**（AI の非決定性をブロッキングのゲートに持ち込まないため）。cdk diff がゲートに繋がっていないのは、AWS 側の一時障害でマージが止まらないようにするためである。
...
dev へ入る経路はこの1本だけである。dev を触るものはスケジュール実行の destroy も含めて同じ直列化グループに入れる（「変更可能境界」参照）。

## なぜこの構造・方式を選んだか（採用理由）

| 判断                                                                                                                   | なぜ                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| deploy を **main へのマージ後**に行う                                                                                  | マージまでの経路に最も時間のかかる工程を置かないため（「設計のポイント」参照）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| docs のみの PR でも落ちうる検査は、全体検査とは別に**変更種別に関わらず常に実行**する                                  | 全体検査は docs のみの PR でスキップされる。docs を直すことで壊れる検査をそこに入れると、**その検査が最も要る場面でこそ走らない**（下表参照）。しかも被害は後続の無関係な PR に出る。分けて置けるのは、いずれも root の依存だけで完結し型情報を要らないため（3階層分の依存が要る全体検査と違い軽い）                                                                                                                                                                                                                                                                                                        |
...
| cdk diff を **cicd-gate に繋がない**                                                                                   | 実環境への疎通をマージ可否に持ち込むと、AWS 側の一時障害でマージが止まる。diff は判断材料であって合否ではない                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| PR 説明の最終稿は AI が書き換える（push 直後の `--fill` は PR を存在させるためのもの）                                 | commit メッセージの機械的な要約（`--fill`）は「何を・なぜ変えたか」を人間の言葉で語れない。cicd-gate 通過後に AI が差分全体を見て本文を書き直すことで、pr-review-policy が求める「PR説明の記載」の質を上げる。push 直後は差分がまだ変わりうるため、他ジョブが PR 番号を参照できるよう `--fill` で即座に PR を存在させておく                                                                                                                                                                                                                                                                                 |
...
| AI は **advisory**（マージ可否の required check には含めない）                                                         | AI の判断は非決定的（同じ差分でも出力が揺れうる）。cicd-gate が守るのは決定論——required check は一意に決まる作業の結果でなければならない。AI を cicd-gate に入れると、Claude の障害やブレでマージが止まる                                                                                                                                                                                                                                                                                                                                                                                                   |
| `policy` ラベルの付与を AI に預けず、AI ステップより前の決定論ステップで行う                                           | 判断基準（Policy・CLAUDE.md）が壊れると、以後すべての AI 判断が静かに歪む。対象かどうかはパスだけで決まる決定論的な判定なので、AI の見落としに賭けず機械に落とす。対象パスの正は pr-review-policy の policy 行で、ワークフローの grep はその写し。AI 側（pr-label）にも同じ判定を置くのは、手元で単体実行したときにも再現するため                                                                                                                                                                                                                                                                           |
...
| merge に要る `contents: write` を AI ジョブに持たせず、auto-merge を別ジョブに閉じ込める                               | GitHub Actions の権限は job 単位。AI ジョブは差分（ブランチ作者が書ける文字列）を読むため、プロンプトインジェクションが通った場合を前提に設計する。merge 権限を同居させると injection → `git push` で main を改ざんする経路が構造的に開く。AI ジョブの `contents` を read に固定し、決定論の auto-merge だけを別ジョブ（`contents: write`）に置いて塞ぐ                                                                                                                                                                                                                                                     |
...
| 却下案                                                                            | 理由                                                                                                                                                                                                                                                            |
| --------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| マージ**前**に dev へ deploy し、その成功をレビューの Entry Criteria とする       | CDK は deploy しないと分からないエラーが多く、レビューのやり直しを防げる利点はある。だがマージの手前に最も遅い工程が居座り、変更が小さいほど待ち時間の割合が大きくなる。却下によって失う利点（deploy 成功の事前確認）は「既知の制約」を参照                     |
| マージ前に deploy し、その結果を cicd-gate の needs から外して advisory にする    | deploy 結果が誰の判断にも使われないまま dev を奪い合い続ける。待ち時間だけが残る                                                                                                                                                                                |
| トピックブランチを dev へ手動 deploy できる `workflow_dispatch` を用意する        | dev へ入る経路が2つになり、「dev にいま何があるか」が一意に決まらなくなる                                                                                                                                                                                       |
| cdk diff を cicd-gate の needs に入れる                                           | 実環境への疎通がマージ可否に混ざり、AWS 側の一時障害でマージが止まる                                                                                                                                                                                            |
...
| PR 説明を `--fill`（commit メッセージの機械的な要約）のままにし、AI に書かせない  | 要約は「何を・なぜ変えたか」を人間の言葉で語れない                                                                                                                                                                                                              |
...
| --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **Dependabot が見るのは GitHub Actions だけ**（npm を足さない）                                                 | GitHub は Dependabot の実行に secrets を渡さない。npm を足すと更新 PR の cdk diff が AWS 認証で落ちる。deploy はマージ後にあるため**ゲートは赤くならない**が、毎回失敗する advisory ジョブは誰も見なくなる。npm 依存は代わりに検査側で見る（採用理由の表を参照）                          | Dependabot 用の secrets の設定                                                               |
...
| **リポジトリ設定でブランチの最新取り込みが required になっている**（Require branches to be up to date）         | 検査したコードとマージされるコードがずれる。deploy がマージの後ろにある本設計では、マージ前に main の破損を防ぐのはこの設定だけになっている                                                                                                                                               | GitHub リポジトリ設定で再登録                                                                |
| **cicd-gate がリポジトリ設定で required check として登録されている**                                            | required 化していないと「マージ可否を cicd-gate 1つに集約する」という TL;DR の決定が保証されない                                                                                                                                                                                          | GitHub リポジトリ設定で required check を再登録                                              |
...
| レビュー時点で deploy が未検証     | 速さと引き換えの対価。CDK は deploy しないと分からないエラーが多いが、PR の時点では cdk diff までしか見えない。IAM の制約・リソース名の衝突・サービス上限は main へ入った後に初めて分かる                                                                                                                                                                                                                                                 |
