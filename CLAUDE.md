...
- **判断ログの記録**：その回で (a) 人間に判断・実装を矯正された、または (b) トレードオフを伴う非自明な自律判断を下したときは、回答末尾でその旨に触れ、[docs/policy/judgment-log.md](docs/policy/judgment-log.md) にテンプレートに沿って1エントリ追記すること（原則を実データから育てるための素材収集）
